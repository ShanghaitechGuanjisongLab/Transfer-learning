...
>（A）热图展示初始/迁移的全局响应模式。整体响应强度。（B）线图比较初始/迁移的全细胞平均曲线，基本重合。（C）上：比较初始/迁移的 1s z-score。下：比较初始/迁移鼠的活跃细胞占比。迁移鼠活跃细胞占比多，但z-score却更低。（D）图示初始/迁移的细胞活跃格局。
