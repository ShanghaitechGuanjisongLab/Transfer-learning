% Fig54D model first formal training unit L5 response heatmap.

svgName = '中文图Fig54D_ModelFirstFormalUnitDecisionHeatmap.svg';
...
assignin('base', 'Fig54D_ModelDecisionHeatmapData', HeatmapData);
assignin('base', 'Fig54D_ModelDecisionHeatmapRunInfo', RunInfo);
assignin('base', 'Fig54D_ModelDecisionHeatmapPlotData', PlotData);
assignin('base', 'Fig54D_ModelDecisionHeatmapSvgPath', svgPath);
...
if height(transferRows) ~= Params.NumMice
	error('Fig54D:TransferHeatmapSeedCountMismatch', 'Expected %d Transfer heatmap seed rows, got %d.', Params.NumMice, height(transferRows));
end

condRow = Cond(Cond.Name == "Transfer", :);
if height(condRow) ~= 1
	error('Fig54D:MissingTransferCondition', 'Expected exactly one Transfer condition row.');
end
