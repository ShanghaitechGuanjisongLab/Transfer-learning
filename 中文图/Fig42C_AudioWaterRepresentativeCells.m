% 中文图42C：声水 Naive/Learned 代表性细胞曲线
...
if ~okNeg1 || ~ok0 || ~ok1
	error('Fig42C:BadTimeIndex', 'Cannot find samples close to -1s, 0s and 1s.');
end
...
if ~all(ismember(["CellUID", "Mouse"], string(cellMeta.Properties.VariableNames)))
	error('Fig42C:MissingCellMeta', 'DS.Cells lacks CellUID/Mouse metadata.');
end
...
if size(X, 3) < 2
	error('Fig42C:BadNTATS', 'Expected 2 lanes for Naive/Learned AudioWater.');
end
if ~all(ismember(["CellUID", "Mouse"], string(S.Properties.VariableNames)))
	error('Fig42C:MissingMeta', 'NtatsCellStrip output lacks CellUID/Mouse.');
end
...
if ~any(posMask)
	error('Fig42C:NoPositiveCell', 'No cell satisfies the positive representative-cell criterion.');
end
if ~any(negMask)
	error('Fig42C:NoNegativeCell', 'No cell satisfies the negative representative-cell criterion.');
end
...
f = figure('Color', 'w', 'Name', '中文图42C 声水代表性细胞');
f.Units = 'centimeters';
f.Position(3:4) = [6, 4];
...
ax1 = axes(f, 'Units', 'normalized', 'Position', [0.14 0.20 0.28 0.46]);
...
ax2 = axes(f, 'Units', 'normalized', 'Position', [0.60 0.20 0.30 0.46]);
...
annotation(f, 'line', [0.36 0.44], [0.90 0.90], 'Color', colorLearn, 'LineWidth', 1);
annotation(f, 'textbox', [0.44 0.865 0.15 0.07], 'String', 'Learned', 'EdgeColor', 'none', ...
	'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'FontSize', 6);
descriptorText = annotation(f, 'textbox', [0.57 0.755 0.41 0.17], 'String', sprintf('Post-stimulus\nactivation\nreversal'), 'EdgeColor', 'none', ...
...
descriptorText.FontSize = 6;
descriptorText.FontWeight = 'bold';
svgPath = TransferLearning.StandardFigureSvgPath('中文图Fig42C_AudioWaterRepresentativeCells.svg');
...
assignin('base', 'Fig42C_CellStrip', S);
...
function baseLearnShift = iComputeLearnBase(~, yNaive, yLearn, baseMask, gap)
...
if isnumeric(nt)
	...
	error('Fig42C:BadNTATSContainer', 'Unsupported NTATS container type: %s', class(nt));
end