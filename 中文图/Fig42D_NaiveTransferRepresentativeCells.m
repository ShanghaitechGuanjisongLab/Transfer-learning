...
if ~okNeg1 || ~ok0 || ~ok1
	error('Fig42D:BadTimeIndex', 'Cannot find samples close to -1s, 0s and 1s.');
end
...
if ~all(ismember(["CellUID", "Mouse"], string(cellMeta.Properties.VariableNames)))
	error('Fig42D:MissingCellMeta', 'DS.Cells lacks CellUID/Mouse metadata.');
end
...
if ~any(posMask)
	error('Fig42D:NoPositiveCell', 'No cell satisfies the positive criterion.');
end
if ~any(negMask)
	error('Fig42D:NoNegativeCell', 'No cell satisfies the negative criterion.');
end
...
f = figure('Color', 'w', 'Name', '中文图42D Naive Learned 代表性细胞');
f.Units = 'centimeters';
f.Position(3:4) = [6, 4];
...
annotation(f, 'line', [0.36 0.44], [0.90 0.90], 'Color', colorLearned, 'LineWidth', 1);
annotation(f, 'textbox', [0.44 0.865 0.15 0.07], 'String', 'Learned', 'EdgeColor', 'none', ...
	'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'FontSize', 6);
descriptorText = annotation(f, 'textbox', [0.57 0.755 0.41 0.17], 'String', sprintf('Continued\npost-stimulus\nactivation change'), 'EdgeColor', 'none', ...
...
svgPath = TransferLearning.StandardFigureSvgPath('中文图Fig42D_NaiveTransferRepresentativeCells.svg');
...
assignin('base', 'Fig42D_PickedCells', picked);
...
if isnumeric(nt)
	...
	error('Fig42D:BadNTATSContainer', 'Unsupported NTATS container type: %s', class(nt));
end