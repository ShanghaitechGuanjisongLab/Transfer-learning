function Data = Fig383BC_THInhibitCtrlActiveCalciumData()
% Shared data for Fig383B/C: learned AudioWater active cells in first Transfer LightWater session.
...
if ~ok1s
	error('Fig383BC:No1s', 'Cannot find sample close to 1 s.');
end
...
if isempty(ctrl.Trace) || isempty(th.Trace)
	error('Fig383BC:EmptyData', 'No learned-active cells found in Ctrl or TH group.');
end
