% Fig55D model first formal training unit L5 response heatmap.

svgName = '中文图Fig55D_ModelFirstFormalUnitDecisionHeatmap.svg';
...
assignin('base', 'Fig55D_ModelDecisionHeatmapData', HeatmapData);
assignin('base', 'Fig55D_ModelDecisionHeatmapRunInfo', RunInfo);
assignin('base', 'Fig55D_ModelDecisionHeatmapPlotData', PlotData);
assignin('base', 'Fig55D_ModelDecisionHeatmapSvgPath', svgPath);
...
if height(transferRows) ~= Params.NumMice
	error('Fig55D:TransferHeatmapSeedCountMismatch', 'Expected %d Transfer heatmap seed rows, got %d.', Params.NumMice, height(transferRows));
end

condRow = Cond(Cond.Name == "Transfer", :);
if height(condRow) ~= 1
	error('Fig55D:MissingTransferCondition', 'Expected exactly one Transfer condition row.');
end
