...
safeName = matlab.lang.makeValidName(queryXlsx);
cacheFile = fullfile(cacheDir, 'KnowledgeChange_' + safeName + '.mat');
if isfile(cacheFile)
...
pairs = table( ...
	...
	["Naive 💡💧"; "Learned 🔊💧"; "T-start 💡💧"; "Learned 🔊💧"], ...
	["Learned 💡💧"; "T-start 💡💧"; "T-learned 💡💧"; "T-learned 💡💧"], ...
	["Naive→Learned"; "Learned→T-start"; "T-start→T-learned"; "Learned→T-learned"], ...
...
pairStats = table('Size', [height(pairs), 5], ...
	'VariableTypes', {'string', 'string', 'string', 'cell', 'cell'}, ...
	'VariableNames', {'LeftPhase', 'RightPhase', 'TransitionKey', 'MousePairs', 'UNS'});
pairStats{:, {'LeftPhase', 'RightPhase', 'TransitionKey'}} = pairs{:, {'LeftPhase', 'RightPhase', 'TransitionKey'}};
...
Data.CacheInfo = struct('NumPair', height(pairStats), 'TransitionKeys', pairStats.TransitionKey.');
CachedData = Data;
