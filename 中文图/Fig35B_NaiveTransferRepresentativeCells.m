...
if ~okNeg1 || ~ok0 || ~ok1
	error('中文图35B:BadTimeIndex', 'Cannot find samples close to -1s, 0s and 1s.');
end
...
if ~all(ismember(["CellUID", "Mouse"], string(cellMeta.Properties.VariableNames)))
	error('中文图35B:MissingCellMeta', 'DS.Cells lacks CellUID/Mouse metadata.');
end
...
if ~any(posMask)
	error('中文图35B:NoPositiveCell', 'No cell satisfies the positive criterion.');
end
if ~any(negMask)
	error('中文图35B:NoNegativeCell', 'No cell satisfies the negative criterion.');
end
...
f = figure('Color', 'w', 'Name', '中文图35B Naive Learned 代表性细胞');
f.Units = 'centimeters';
...
svgPath = fullfile(outDirUNC, '中文图Fig35B_NaiveLearnedRepresentativeCells.svg');
...
assignin('base', 'Fig35B_PickedCells', picked);
...
if isnumeric(nt)
	...
	error('中文图35B:BadNTATSContainer', 'Unsupported NTATS container type: %s', class(nt));
end