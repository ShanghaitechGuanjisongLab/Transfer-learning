...
patches = MATLAB.Graphics.MultiShadowedLines( ...
	...
	X=double(Data.XSec(:)), ...
	EdgeColors=[TransferLearning.LearnedColor; TransferLearning.NaiveColor; TransferLearning.ColorA; TransferLearning.ColorB], ...
	LineStyles=Data.PhaseLineStyles, ...
	Ax=ax);

legendProxy = gobjects(1, numel(Data.Phases));
for iPhase = 1:numel(Data.Phases)
	legendProxy(iPhase) = plot(ax, nan, nan, Data.PhaseLineStyles(iPhase), 'Color', Colors(iPhase, :), 'LineWidth', 2);
end
