function Data = Fig71_BaselineConvergenceCache(varargin)
if nargin > 1
...
function Cache = iBuildCache()
	...
	for iPhase = 1:numel(phaseNames)
		...
		if ~isfield(groupNts, phaseName)
			error('Fig71:MissingPhase', 'Phase %s is missing from QueryNTS result.', phaseName);
		end
	...
	if mod(size(sampleSignal, 1), 30) ~= 0
		error('Fig71:InvalidLearnedTrials', 'LearnedAudio rows cannot be reshaped into 30-trial stacks.');
	end
	...
	Cache.PhaseLineStyles = phaseLineStyles;
...
function varMat = iVarianceMatrix(cellSummary)
	...
	elseif any(strcmp(cellSummary.Properties.VariableNames, 'std_TrialSignal'))
		...
		error('Fig71:MissingVariance', 'groupsummary output does not contain var_TrialSignal or std_TrialSignal.');
	end
...
function cmap = iTrialColormap(nTrial)
	if nTrial <= 1
		cmap = TransferLearning.ColorA;
		return;
	end
	startColor = TransferLearning.ColorA;
	endColor = TransferLearning.ColorB;
