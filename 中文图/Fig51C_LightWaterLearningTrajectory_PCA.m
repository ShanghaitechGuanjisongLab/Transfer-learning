...
f = figure('Color', 'w', 'Name', '中文图51C LightWater learning trajectory PCA');
f.Units = 'centimeters';
f.Position(3:4) = [12, 8];
...
ax1 = nexttile(tlo, 1);
iPlotMouseTrajectory(ax1, Naive.Points, Naive.Explained, groupColors(1, :), 'Naive');

ax2 = nexttile(tlo, 2);
iPlotMouseTrajectory(ax2, Continual.Points, Continual.Explained, groupColors(2, :), 'Continual');
...
svgPath = '中文图Fig51C_LightWaterLearningTrajectory_PCA.svg';
svgPath = TransferLearning.ExportStandardFigure(f, 2, svgPath);
fprintf('Wrote: %s\n', svgPath);
TransferLearning.Fig51.PrintStateSpaceUsageCounts("Fig51C", Counts, "Representative");

assignin('base', 'Fig51C_NaiveTrajectory', Naive);
assignin('base', 'Fig51C_ContinualTrajectory', Continual);
assignin('base', 'Fig51C_Counts', Counts.Representative);
