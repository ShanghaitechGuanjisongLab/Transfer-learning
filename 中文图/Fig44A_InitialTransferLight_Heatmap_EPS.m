...
for ax = axesAll
	...
	ax.YTick = [];
	ax.YTickLabel = {};
	ax.XTickLabel(ismember(ax.XTick , [0 1])) = {'Light', 'Water'};
...
% EPS 输出：应用标准样式后，用 -depsc 导出
TransferLearning.ApplyStandardExportStyle(f, 2);

% 统一所有文本为 Arial（确保 Adobe Illustrator 兼容）
...
% 也对布局子对象设置，避免 findall 遗漏
layoutHandles = findall(f, '-isa', 'matlab.graphics.layout.TiledChartLayout');
for Lh = layoutHandles(:)'
	labelProps = {'Title', 'XLabel', 'YLabel'};
	for prop = labelProps
		if isprop(Lh, prop{1}) && isgraphics(Lh.(prop{1}))
			Lh.(prop{1}).FontName = 'Arial';
		end
	end
end
...
epsFileName = '中文图Fig44A_InitialTransferLight_Heatmap.eps';
epsPath = fullfile(outDirUNC, epsFileName);
print(f, epsPath, '-depsc');
fprintf('Wrote: %s\n', epsPath);
