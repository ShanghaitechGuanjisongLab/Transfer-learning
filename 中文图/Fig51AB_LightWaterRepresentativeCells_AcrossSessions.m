...
Data = TransferLearning.Fig51.BuildStateSpaceSummary(false, UniExp.Flags.ZScore);
Naive = Data.Representative.NaiveCell;
Continual = Data.Representative.ContinualCell;
xsSec = Data.XsSec;
plotMask = xsSec >= 0 & xsSec <= 2;
xsPlot = xsSec(plotMask);
nCol = max(size(Naive.Signals, 1), size(Continual.Signals, 1));
figWidthCm = 12;
figHeightCm = 8;
%% 

f = figure('Color', 'w', 'Name', '中文图51AB LightWater representative cells across sessions');
f.Units = 'centimeters';
...
continualSignals = Continual.Signals(:, plotMask);
yLimNaive = [min(naiveSignals, [], 'all'), max(naiveSignals, [], 'all')];
yLimContinual = [min(continualSignals, [], 'all'), max(continualSignals, [], 'all')];
...
for iSess = 1:nCol
	...
	if iSess <= size(Naive.Signals, 1)
		...
		iPlotSessionTile(ax, xsPlot, Naive.Signals(iSess, plotMask), double(Naive.SessionTable.Performance(iSess)), groupColors(1, :), ...
			headerText, iSess, yLimNaive, iSess == 1);
	else
		axis(ax, 'off');
	end
	if iSess == 1
...
for iSess = 1:nCol
	...
	if iSess <= size(Continual.Signals, 1)
		if iSess == 1
			headerText = sprintf('Continual\nCell %u', Continual.CellUID);
		else
			headerText = '';
		end
		iPlotSessionTile(ax, xsPlot, Continual.Signals(iSess, plotMask), double(Continual.SessionTable.Performance(iSess)), groupColors(2, :), ...
			headerText, iSess, yLimContinual, iSess == 1);
	else
		axis(ax, 'off');
	end
	if iSess == 1
...
svgPath = '中文图Fig51AB_LightWaterRepresentativeCells_AcrossSessions.svg';
svgPath = TransferLearning.ExportStandardFigure(f, 1, svgPath);
fprintf('Wrote: %s\n', svgPath);

assignin('base', 'Fig51AB_NaiveRepresentativeCell', Naive);
assignin('base', 'Fig51AB_ContinualRepresentativeCell', Continual);

function iPlotSessionTile(ax, xsPlot, y, perf, baseColor, headerText, sessionIndex, yLim, showY)
hold(ax, 'on');
	h = plot(ax, xsPlot, y, 'Color', baseColor, 'LineWidth', 1, 'HandleVisibility', 'off');
	setappdata(h, 'TransferLearningPreserveLineWidth', true);
	v = xline(ax, 1, '--', 'LineWidth', 1, 'HandleVisibility', 'off');
