% 中文图343H：TH抑制与对照组首会话重激活率
%
% 样式模仿英文图3J中的上下双 bar tile：
% - 上下双 tile
% - 白底
...
svgName = "中文图Fig343H_THInhibitVsCtrl_FirstSessionReactivation.svg";
...
if isempty(RCtrl) || isempty(RTH)
	error('Fig343H:Empty', 'No valid mice for first-session reactivation comparison.');
end
...
if isempty(xCtrl23) || isempty(xTH23) || isempty(xCtrl5) || isempty(xTH5)
	error('Fig343H:NoFiniteData', 'No finite layer-wise reactivation values in one or both groups.');
end

p23 = ranksum(xCtrl23, xTH23);
p5 = ranksum(xCtrl5, xTH5);

fprintf('\n=== Fig343H MOp2/3 reactivation ===\n');
...
nexttile(layout, 1);
[~, options23, bars23, errorBars23] = UniExp.BarScatterCompare({xCtrl23(:), xTH23(:)}, false, compareGroup, 'AsteriskThreshold', 0.05);
iStyleTile(gca, bars23, errorBars23, false, 'MOp2/3');
iApplyPText(options23, p23);

nexttile(layout, 2);
[~, options5, bars5, errorBars5] = UniExp.BarScatterCompare({xCtrl5(:), xTH5(:)}, false, compareGroup, 'AsteriskThreshold', 0.05);
...
assignin('base', 'Fig343H_ReactivationP23', p23);
assignin('base', 'Fig343H_ReactivationP5', p5);
...
if ~ok1s
	error('Fig343H:No1s', 'Cannot find sample close to 1 s.');
end
