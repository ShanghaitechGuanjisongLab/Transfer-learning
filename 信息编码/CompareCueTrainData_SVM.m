%% CompareCueTrainData_SVM.m
% CONTROL experiment: same pipeline as CompareCueTrainData.m (3 training
% configs x [MI, Stage1 TRAIN-SET (incl. Naive AW; NOT strict held-out), Stage2 Transfer LW (TRUE held-out)]) but the decoder is a
% NON-LINEAR RBF-SVM instead of GLM naive-Gaussian.
% Purpose: test whether the core conclusions are robust to decoder choice:
%   (1) no pre-stimulus information (t<0 chance)
%   (2) signal appears only after stimulus onset
%   (3) in-task decodable, weak/asymmetric Transfer generalization
% P(audio) = 1./(1+exp(score)); score>0 -> light (same convention as GLM).
% Run via MATLAB MCP.

%% 0. Setup (identical to CompareCueTrainData.m)
...
tVec = xs(tIdxFull);
nTfull = numel(tVec);
fprintf('=== CUE training-data comparison: NON-LINEAR RBF-SVM ===\n');
...
%% 2. Per-mouse data (identical to CompareCueTrainData.m)
...
%% 3. Decode with RBF-SVM (train CV MI + tendency on held-out tests)
cfgNames = {'AudioWater+AudioOnly+LightOnly','AudioOnly+LightOnly','AudioOnly+LightOnly (miss)'};
miCV = nan(nValid, 3, nTfull);
pSt1 = nan(nValid, 3, 4, nTfull);
pSt2 = nan(nValid, 3, 2, nTfull);
% scale-free metrics for fair comparison with GLM (balacc per stage/type)
balSt1 = nan(nValid, 3, 4, nTfull);   % [AO, LO, AH, AM]
balSt2 = nan(nValid, 3, 2, nTfull);   % [LH, LM]
for i = 1:nValid
    r = resAll{i};
    for cfg = 1:3
        ...
        for iT = 1:nTfull
            Ftr = Xtr(:,:,iT);
            [sOOF, pOOF] = iCvPredictSVM(Ftr, ytr, K);
            miCV(i,cfg,iT) = iMIFromLabels(pOOF, ytr);
            pAOOF = 1./(1+exp(sOOF));   % P(audio) out-of-fold (scale-compressed for SVM)
            pSt1(i,cfg,1,iT) = mean(pAOOF(typ==1));
            pSt1(i,cfg,2,iT) = mean(pAOOF(typ==2));
            % AO/LO: single-class tendency; accuracy = fraction of that class predicted correctly
            balSt1(i,cfg,1,iT) = mean(pOOF(typ==1)==0);   % AudioOnly -> should be audio(0)
            balSt1(i,cfg,2,iT) = mean(pOOF(typ==2)==1);   % LightOnly -> should be light(1)
            bal = iBalanceTrain(ytr);
            [sN,~] = iSvmDecode(Ftr(bal,:), ytr(bal), r.Xn(:,:,iT));
            [sT,~] = iSvmDecode(Ftr(bal,:), ytr(bal), r.Xt(:,:,iT));
            ...
            % Stage2 (Transfer LW, all light): balacc = how often predicted light(1)
            %   - LH: predict-light accuracy on hit trials; LM: predict-light accuracy on miss
            balSt2(i,cfg,1,iT) = mean(double(sT>0)==1 & r.behT==1);   % hit trials predicted light
            balSt2(i,cfg,2,iT) = mean(double(sT>0)==0 & r.behT==0);   % miss trials predicted audio
        end
    end
end

%% 4. Console summary of the 3 core conclusions (SVM version, scale-free)
fprintf('\n===== SVM results: 3 core conclusions (MI + balacc, scale-free) =====\n');
for cfg = 1:3
    v = mean(miCV(:,cfg,:),1,'omitnan');
    [pk, pi_] = max(v);
    b1 = mean(squeeze(balSt1(:,cfg,1,:)),1,'omitnan');
    b2 = mean(squeeze(balSt1(:,cfg,2,:)),1,'omitnan');
    fprintf('cfg%d: MI peak=%.3f@%.2f | pre MI=%.3f | Stage1 balacc AO=%.3f LO=%.3f (t=%.2f)\n', ...
        cfg, pk, tVec(pi_), mean(v(tVec<0)), b1(end), b2(end), tVec(end));
    for ti = [5 12 16]
        fprintf('   t=%+5.2f | MI %.3f | AO %.3f LO %.3f AH %.3f AM %.3f | LH %.3f LM %.3f\n', ...
            tVec(ti), mean(miCV(:,cfg,ti),'omitnan'), ...
            mean(pSt1(:,cfg,1,ti),'omitnan'), mean(pSt1(:,cfg,2,ti),'omitnan'), ...
            mean(pSt1(:,cfg,3,ti),'omitnan'), mean(pSt1(:,cfg,4,ti),'omitnan'), ...
            mean(pSt2(:,cfg,1,ti),'omitnan'), mean(pSt2(:,cfg,2,ti),'omitnan'));
    end
end
fprintf('NOTE: SVM tendency uses raw sigmoid(score); RBF-SVM scores are compressed near 0,\n');
fprintf('so absolute tendency is attenuated vs GLM. Use MI / balacc for fair comparison.\n');

%% 5. Figure 3x3 (SVM)
f = figure('Name','Cue decoder: RBF-SVM training-data comparison (control)','Color','w','Position',[40 40 1320 940]);
...
for cfg = 1:3
    for c = 1:3
        ...
        if c==1
            ...
            ylabel(ax,'P(audio) (0=light,1=audio)');
        end
...
fprintf('\nDone. SVM control complete.\n');

% auto-save the 3x3 figure
outDir = fullfile(thisDir, '_figcheck');
if ~exist(outDir, 'dir'); mkdir(outDir); end
exportgraphics(f, fullfile(outDir, 'Cue_decoder__SVM_control__3_scripts_.png'), 'Resolution', 200);
fprintf('saved SVM control figure to _figcheck.\n');
...
% RBF-SVM, labels ytr in {0,1}; score>0 -> predicted +1 (=light), <0 -> audio.
yy = 2*ytr - 1;              % audio=-1, light=+1
mdl = fitcsvm(Ftr, yy, 'KernelFunction','rbf', 'KernelScale','auto', ...
    'BoxConstraint', 1, 'Standardize', true);
[~, sc2] = predict(mdl, Fte);   % n x 2 (scores per class)
score = sc2(:, 2);              % score of +1 (light) class; >0 -> light
pred = double(score > 0);       % 1 = light, 0 = audio
end
...
function ba = iBalAcc(pred, lab)
% balanced accuracy: pred/lab in {0,1}; guard against empty class
a0 = mean(pred(lab==0)==0); a1 = mean(pred(lab==1)==1);
ba = mean([a0, a1], 'omitnan');
end
