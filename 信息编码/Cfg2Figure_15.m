%% Cfg2Figure_15.m
% 1.5s 时间窗版简版图（2x2）：行1 = Cue cfg2 (AudioOnly+LightOnly)，行2 = Choice cfg2 (AudioWater)
% 列 = [Stage1 prob-tendency, Stage2 prob-tendency]（无 MI 列）
%   Cue Stage1 只留 audio only / light only；Choice Stage1 只留 audio hit / audio miss；Stage2 = light hit/miss。
% 数据：与主脚本同源，仅时间窗改为 [-0.957, +1.468] s（20 点）。rng(42)。
...
nTime = numel(xs);
tIdxFull = find((xs >= -1) & (xs <= 1.5));   % 延长到 +1.5s
tVec = xs(tIdxFull);
nTfull = numel(tVec);
...
%% 1. CUE cfg2 (AO+LO): Stage1(AO/LO via OOF) + Stage2(light hit/miss via held-out Transfer)
...
for iM = 1:numel(miceAll)
    ...
    if ~okT(trA) || ~okT(trL) || ~okT(teTrans); continue; end
    trA = trA(~isnan(trA.Behavior), :); trL = trL(~isnan(trL.Behavior), :); teTrans = teTrans(~isnan(teTrans.Behavior), :);
    s2 = [trA; trL];
    ...
    if isempty(X2) || isempty(Xt); continue; end
    if doBaselineNorm
        bI = find(tVec < 0);
        X2 = iBaselineNorm(X2, bI); Xt = iBaselineNorm(Xt, bI);
    end
    nCue = nCue + 1;
    resCue{nCue} = struct('X2',X2,'Xt',Xt,'y2',y2,'typ2',typ2,'behT',iTrialLabel(teTrans,'Behavior'));
end
resCue = resCue(1:nCue);
fprintf('Cue cfg2 valid mice: %d\n', nCue);
pSt1Cue = nan(nCue, 2, nTfull); pSt2Cue = nan(nCue, 2, nTfull);
for i = 1:nCue
    r = resCue{i};
    for iT = 1:nTfull
        Ftr = r.X2(:,:,iT);
...
%% 2. CHOICE cfg2 (AudioWater): Stage1(hit/miss via OOF) + Stage2(light hit/miss via held-out Transfer)
...
for iM = 1:numel(miceAll)
    ...
    nCh = nCh + 1;
    resCh{nCh} = struct('Xtr2',Xtr2,'Xte',Xte,'behTr2',behTr2,'behTe',behTe);
end
resCh = resCh(1:nCh);
fprintf('Choice cfg2 valid mice: %d\n', nCh);
pSt1Ch = nan(nCh, 2, nTfull); pSt2Ch = nan(nCh, 2, nTfull);
for i = 1:nCh
    r = resCh{i};
    okB = ~isnan(r.behTr2);
    for iT = 1:nTfull
...
for r = 1:2
    for c = 1:2
        ...
        if c == 1   % Stage1
            if r == 1
                P = pSt1Cue; stN = {'audio only','light only'}; stC = {[0.30 0.60 0.20], [0.70 0.30 0.70]};
                ylbl = 'P(audio) tendency (0=light,1=audio)';
            else
                P = pSt1Ch; stN = {'audio hit','audio miss'}; stC = {[0.85 0.33 0.10], [0.10 0.45 0.70]};
            ...
            stN = {'light hit','light miss'}; stC = {[0.85 0.33 0.10], [0.10 0.45 0.70]};
            for s = 1:2
                v = squeeze(P(:,s,:));
                if all(isnan(v(:))); continue; end
                mn = mean(v,1,'omitnan'); se = std(v,0,1,'omitnan')/sqrt(sum(~isnan(v(:,1))));
