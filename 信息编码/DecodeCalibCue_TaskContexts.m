%% DecodeCalibCue_TaskContexts.m
% TRIAL script - the original DecodePreTransfer_Cue.m is NOT modified.
%
% Cue decoder (Light vs Audio) trained ONLY on AudioOnly + LightOnly trials
% from LAu/LAuW calibration blocks (both classes come from the SAME
% calibration context, so the task-vs-calibration confound is removed from
% training). Tested on:
%   1. Naive AudioWater      (expect tendency to audio:  P(light) < 0.5)
%   2. Transfer LightWater   (expect tendency to light: P(light) > 0.5)
% Output = tendency-to-light P(light) via sigmoid of decoder score.
...
tVecAll  = xs(tIdxFull);
featSel  = tVecAll >= 0;      % decode features: stimulus-onset only (0,1]
tVec  = tVecAll;              % full axis for display (-1,1, 16 pts)
idxFeat = find(featSel);      % positions of stimulus-onset points
nTfull = numel(tVec);         % 16 (display)
nFeat  = numel(idxFeat);      % 8 (decode)

fprintf('=== Calibration cue decoder -> Naive AW / Transfer LW ===\n');
fprintf('Display window: %.2f-%.2f s (%d pts); decode features: t>=0 (%d pts)\n', tVec(1), tVec(end), nTfull, nFeat);
...
for iM = 1:numel(miceAll)
    m = miceAll(iM);
    % --- calibration training trials (AudioOnly + LightOnly) ---
    calTbl = table();
    ...
    % keep stimulus-onset features only (baseline-normalised where applicable)
    Xtr    = Xtr(:,:,featSel); Xnaive = Xnaive(:,:,featSel); Xtrans = Xtrans(:,:,featSel);
    ...
    resAll{nUsed} = res;
    fprintf('  %-9s cells=%3d  calib(tr)=%d (Au%d/Li%d)  naiveAW=%d  transLW=%d\n', ...
...
%% 3. Decode: tendency-to-light (P) on Naive AW and Transfer LW
methods = {'linear','glm'};
...
for i = 1:nValid
    ...
    okTr = ~isnan(yt);
    for j = 1:nFeat
        iT = idxFeat(j);          % position in full -1..1 axis (NaN before onset)
        Ftr = Xtr(:, :, j);
        FtrOK = Ftr(okTr,:); ytOK = yt(okTr);
        for met = 1:2
            bal = iBalanceTrain(ytOK);
            if met == 1
                [sN, ~] = iLinDecode(FtrOK(bal,:), ytOK(bal), r.Xnaive(:,:,j));
                [sT, ~] = iLinDecode(FtrOK(bal,:), ytOK(bal), r.Xtrans(:,:,j));
            else
                [sN, ~] = iGlmDecode(FtrOK(bal,:), ytOK(bal), r.Xnaive(:,:,j));
                [sT, ~] = iGlmDecode(FtrOK(bal,:), ytOK(bal), r.Xtrans(:,:,j));
            end
            pnL = 1./(1+exp(-sN));   % P(light) on Naive AudioWater
            ptL = 1./(1+exp(-sT));   % P(light) on Transfer LightWater
            pNaive(i,met,1,iT) = mean(pnL,'omitnan');
            pTrans(i,met,1,iT) = mean(ptL,'omitnan');
            for p = 2:3             % hit (p=2), miss (p=3)
                sel = (r.behNaive == (3-p));   % 1 -> hit, 0 -> miss
                if sum(sel) >= 2; pNaive(i,met,p,iT) = mean(pnL(sel)); end
                sel = (r.behTrans == (3-p));
                if sum(sel) >= 2; pTrans(i,met,p,iT) = mean(ptL(sel)); end
            end
        end
    end
end

%% 4. Figures: P(light) on Naive AudioWater vs Transfer LightWater
poolNames = {'all','hit','miss'};
pcols = {[0 0 0], [0.30 0.60 0.20], [0.70 0.30 0.70]};
for met = 1:2
    f = figure('Name', sprintf('Calib cue decoder (%s) - tendency to light', methods{met}), ...
    ...
    for ctx = 1:2
        ...
        xlabel(ax,'Time from stimulus (s)'); ylabel(ax,'P(light)');
        title(ax, ttl, 'FontSize',10, 'FontWeight','normal');
        legend(ax,'Location','northwest','Box','off','FontSize',7);
        box(ax,'off'); ax.FontSize = 8;
    end
    sgtitle(f, sprintf('Cue decoder trained on LAu/LAuW calib (AudioOnly/LightOnly) - %s', methods{met}), ...
        'FontSize', 10);
end

fprintf('\nDone. Calibration cue decoder complete.\n');
