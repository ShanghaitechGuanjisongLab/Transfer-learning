%% DecodePreTransfer_HitMiss_Vacation7.m
% CONTROL comparison of DecodePreTransfer_HitMiss.m on the Vacation7 dataset
% (learned AudioWater, then 7 days OFF, then Transfer LightWater; 6 mice) vs
% the AudioLightBaseline dataset (11 mice, no gap). All analysis logic,
% thresholds (>=3 trials/class) and decoding methods are kept IDENTICAL to
% the original script so the conclusions are directly comparable.
%
% Differences from the original script:
%   1) DS = TransferLearning.Vacation7().
%   2) Data loading uses iQueryZScoreNts (TableQuery + ResampledSignal) instead
%      of DS.QueryNTS, because Vacation7 contains cells outside the
%      registration range whose traces are all-NaN/all-zero; QueryNTS throws
%      on them. Bad rows are dropped (reported), and per-row z-score over the
%      full window was verified identical to UniExp.Flags.ZScore with
%      F0Samples = 1:nTime (max abs diff = 0).
%   3) Figure names are prefixed "V7 ".
%   4) A final summary section prints the key numbers side by side with the
%      AudioLightBaseline conclusions for direct comparison.
%
...
%             AudioOnly + LightOnly; NO Recall)
%   Scheme 2: AudioWater only
%   Scheme 3: AudioWater blocks with Performance > 0.5 only
%   Scheme 4: AudioWater blocks with Performance < 0.6 only
% Test on two stages:
...
% start the parallel pool once (permutation tests below run in parfor over permutations)
...
fprintf('=== Vacation7 CONTROL: Pre-Transfer HIT/MISS (choice) decoder ===\n');
...
% Vacation7 has LAuW blocks only.
...
for iM = 1:numel(miceAll)
    m = miceAll(iM);
    % --- per-stimulus full tables (z-scored), then filter by block ---
    lwT = iQueryZScoreNts(DS, m, 'LightWater');
    aoT = iQueryZScoreNts(DS, m, 'AudioOnly');
    loT = iQueryZScoreNts(DS, m, 'LightOnly');
    awT = iQueryZScoreNts(DS, m, 'AudioWater');
    % --- test: Transfer LightWater (Stage2) ---
    testTbl = table();
    if ~isempty(lwT)
        testTbl = lwT(ismember(uint64(lwT.BlockUID), uint64(testLW)), :);
    end
    if isempty(testTbl) || ~ismember('TrialSignal', string(testTbl.Properties.VariableNames)); continue; end
    % --- Calibration held-out test: AudioOnly + LightOnly (LAuW, no Recall/Final) ---
    teA = table(); teL = table();
    if ~isempty(aoT)
        teA = aoT(ismember(uint64(aoT.BlockUID), uint64(calBlocks)), :);
    end
    if ~isempty(loT)
        teL = loT(ismember(uint64(loT.BlockUID), uint64(calBlocks)), :);
    end
    ...
    if ~isempty(t)
        ...
        if ~isempty(t); t.Cue = zeros(height(t),1); tr1 = [tr1; t]; end %#ok<AGROW>
    end
    t = aoT;
    if ~isempty(t)
    ...
    tr2 = table();
    if ~isempty(awT)
        t = awT(ismember(uint64(awT.BlockUID), uint64(trainAW)), :);
    ...
    tr3 = table();
    if ~isempty(awT)
        t = awT(ismember(uint64(awT.BlockUID), uint64(trainAWperf)), :);
    ...
    tr4 = table();
    if ~isempty(awT)
        t = awT(ismember(uint64(awT.BlockUID), uint64(trainAWlow)), :);
    ...
    fprintf('  %-9s cells=%3d  S1=%d(h%d/m%d) S2=%d(h%d/m%d) S3=%d(h%d/m%d) S4=%d(h%d/m%d) test=%d(h%d/m%d)  calibA=%d(h%d/m%d) calibL=%d(h%d/m%d)\n', ...
...
resM = cell(nValid, 1);
parfor i = 1:nValid   % independent per mouse (parallel; one sliced output cell)
    rng(42 + i);      % per-mouse seed for CV folds / balancing draws
    resM{i} = iDecodeOneMouse(resAll{i}, K, nTfull);
end
% aggregate per-mouse results into the pooled arrays
for i = 1:nValid
    S = resM{i};
    miMain(i,:,:,:,:) = reshape(S.miMain, [1 size(S.miMain)]);
    balMain(i,:,:,:,:) = reshape(S.balMain, [1 size(S.balMain)]);
    pMain(i,:,:,:,:,:) = reshape(S.pMain, [1 size(S.pMain)]);
    miCal(i,:,:,:) = reshape(S.miCal, [1 size(S.miCal)]);
    balCal(i,:,:,:) = reshape(S.balCal, [1 size(S.balCal)]);
    pCal(i,:,:,:,:) = reshape(S.pCal, [1 size(S.pCal)]);
    miIS(i,:,:,:) = reshape(S.miIS, [1 size(S.miIS)]);
    balIS(i,:,:,:) = reshape(S.balIS, [1 size(S.balIS)]);
    miCtrl(i,:,:,:) = reshape(S.miCtrl, [1 size(S.miCtrl)]);
    balCtrl(i,:,:,:) = reshape(S.balCtrl, [1 size(S.balCtrl)]);
    pCtrl(i,:,:,:,:) = reshape(S.pCtrl, [1 size(S.pCtrl)]);
end
...
met = 2;   % GLM only
axMI = []; axTend = [];
f = figure('Name','V7 Hit/miss decoder (AudioWater, GLM) - hit/miss only','Color','w','Position',[60 60 1120 860]);
...
met = 2;
f = figure('Name','V7 Hit/miss Stage1 MI: CV vs in-sample (scheme2, GLM)','Color','w','Position',[60 80 1120 430]);
...
parfor p = 1:Nperm
    rng(234 + p);   % per-iteration seed (parfor)
    ...
    for i = 1:nValid
        ...
        % same balance gate as the main decode loop (V7: several mice have
        % too few misses on AudioWater because they learn it to near-perfect)
        if sum(yb==1) < 3 || sum(yb==0) < 3; continue; end
...
met = 2;
f = figure('Name','V7 Hit/miss MI permutation null test (scheme2, Stage1 & Stage2, GLM)','Color','w','Position',[60 80 1120 430]);
axP = [];
for st = 1:2
...
axCalMI = []; axCalT = [];
met = 2;
f = figure('Name','V7 Hit/miss decoder (AudioWater, GLM) on AudioOnly/LightOnly calib','Color','w','Position',[60 60 1120 860]);
for ci = 1:2
...
parfor p = 1:NpermC
    rng(345 + p);   % per-iteration seed (parfor)
...
met = 2;
f = figure('Name','V7 Hit/miss calibration MI permutation null test (scheme2, AudioOnly & LightOnly, GLM)','Color','w','Position',[60 80 1120 430]);
axCalP = [];
for ci = 1:2
...
parfor p = 1:NpermQ
    rng(567 + p);   % per-iteration seed (parfor)
...
parfor p = 1:NpermQ2
    rng(678 + p);   % per-iteration seed (parfor)
...
met = 2;
f = figure('Name','V7 Cue-invariance of choice decoder (scheme2, GLM) on pooled calib','Color','w','Position',[60 80 1120 430]);
...
%% 5. Summary: key numbers vs AudioLightBaseline (AL) conclusions
% AL reference (10 valid mice, from DecodePreTransfer_HitMiss.m / 关于choice的解码器.md):
%   scheme2 Stage1 balacc = 0.667 (linear) / 0.676 (glm), 10/10 above chance, p=0.002
%   scheme2 Stage2 balacc = 0.513 / 0.524, 9/10 above chance, p=0.022 (weak generalization)
%   scheme2 Stage1 MI = 0.070 / 0.068; Stage2 MI = 0.021 / 0.019
%   Stage1 P(hit) separation flat across the stimulus (pre 0.165 vs post 0.160, linear)
%     -> Stage1 reads a PRE-STIMULUS tonic state, not evoked activity
%   scheme1 (allPre) Stage1 hit/miss MI ~ 0.20 (cue x performance confound)
fprintf('\n========== SUMMARY: Vacation7 vs AudioLightBaseline conclusions ==========\n');
tPre = tVec < 0; tPost = tVec > 0;
for met = 1:2
    if met == 1; metnm = 'linear'; else; metnm = 'glm'; end
    % scheme2 trainable mice (each class >= 3 on AudioWater)
    nTr = 0; s1m = []; s2m = []; s2mBoth = [];
    for i = 1:nValid
        r = resAll{i};
        y2 = r.behTr2(~isnan(r.behTr2));
        ok = sum(y2==1) >= 3 && sum(y2==0) >= 3;
        if ok; nTr = nTr + 1; end
        v1 = squeeze(balMain(i,met,2,1,:));
        if any(~isnan(v1)); s1m(end+1) = mean(v1,'omitnan'); end
        v2 = squeeze(balMain(i,met,2,2,:));
        if any(~isnan(v2))
            s2m(end+1) = mean(v2,'omitnan'); %#ok<AGROW>
            if numel(unique(r.behTe)) == 2 && sum(r.behTe==1) >= 2 && sum(r.behTe==0) >= 2
                s2mBoth(end+1) = mean(v2,'omitnan'); %#ok<AGROW>
            end
        end
    end
    p1 = iSignTestP(s1m, 0.5); k1 = sum(s1m > 0.5);
    p2 = iSignTestP(s2m, 0.5); k2 = sum(s2m > 0.5);
    p2b = iSignTestP(s2mBoth, 0.5); k2b = sum(s2mBoth > 0.5);
    miS1 = squeeze(miMain(:,met,2,1,:)); miS2 = squeeze(miMain(:,met,2,2,:));
    fprintf('[%s] scheme2 trainable mice = %d/%d (others: <3 misses on AudioWater)\n', metnm, nTr, nValid);
    fprintf('    Stage1 balacc mean=%.3f (%d/%d above 0.5, sign test p=%.3f) | MI mean=%.4f peak=%.4f\n', ...
        mean(s1m), k1, numel(s1m), p1, mean(miS1,'all','omitnan'), max(mean(miS1,1,'omitnan')));
    fprintf('    Stage2 balacc mean=%.3f (%d/%d above 0.5, p=%.3f); both-class Stage2 only: mean=%.3f (%d/%d, p=%.3f) | MI mean=%.4f peak=%.4f\n', ...
        mean(s2m), k2, numel(s2m), p2, mean(s2mBoth), k2b, numel(s2mBoth), p2b, ...
        mean(miS2,'all','omitnan'), max(mean(miS2,1,'omitnan')));
    % pre-stimulus vs post-stimulus P(hit) separation (hit minus miss), scheme2 mean over mice
    for st = 1:2
        dPreN = nan(nValid,1); dPostN = nan(nValid,1);
        for i = 1:nValid
            dh = squeeze(pMain(i,met,2,st,1,:)) - squeeze(pMain(i,met,2,st,2,:));
            if all(isnan(dh)); continue; end
            dPreN(i) = mean(dh(tPre),'omitnan');
            dPostN(i) = mean(dh(tPost),'omitnan');
        end
        fprintf('    Stage%d P(hit) hit-miss separation (mean over mice): pre-stim=%.3f post-stim=%.3f\n', ...
            st, mean(dPreN,'omitnan'), mean(dPostN,'omitnan'));
    end
end
% scheme1 confound reference (AL: ~0.20 MI on Stage1)
for met = 1:2
    if met == 1; metnm = 'linear'; else; metnm = 'glm'; end
    mi1 = squeeze(miMain(:,met,1,1,:));
    fprintf('[%s] scheme1(allPre) Stage1 hit/miss MI mean=%.4f peak=%.4f (AL ref ~0.197/0.196; confounded)\n', ...
        metnm, mean(mi1,'all','omitnan'), max(mean(mi1,1,'omitnan')));
end
% scheme3/4 reference (AL scheme4: balacc ~0.70 Stage1, no Stage2 generalization)
for met = 1:2
    if met == 1; metnm = 'linear'; else; metnm = 'glm'; end
    for sc = [3 4]
        v1 = squeeze(balMain(:,met,sc,1,:)); v2 = squeeze(balMain(:,met,sc,2,:));
        if all(isnan(v1(:))); fprintf('[%s] scheme%d: no trainable mouse\n', metnm, sc); continue; end
        fprintf('[%s] scheme%d Stage1 balacc=%.3f | Stage2 balacc=%.3f\n', ...
            metnm, sc, mean(v1,'all','omitnan'), mean(v2,'all','omitnan'));
    end
end
% calibration (scheme2 decoder on AO/LO) reference, GLM
vMA = squeeze(miCal(:,2,1,:)); vML = squeeze(miCal(:,2,2,:));
fprintf('[glm] calibration held-out MI: AudioOnly mean=%.4f | LightOnly mean=%.4f\n', ...
    mean(vMA,'all','omitnan'), mean(vML,'all','omitnan'));

fprintf('\nDone. Vacation7 HIT/MISS decoder (control vs AudioLightBaseline) complete.\n');

% ==================== Local Functions ====================

function S = iDecodeOneMouse(r, K, nTfull)
% Full per-mouse decode (all schemes x methods x stages), matching exactly
% the body of the original DecodePreTransfer_HitMiss.m loop. Returned as a
% struct so the parfor loop writes only a single sliced variable resM{i}.
miMain = nan(2, 4, 2, nTfull);
balMain = nan(2, 4, 2, nTfull);
miCtrl = nan(2, 2, nTfull);
balCtrl = nan(2, 2, nTfull);
pMain = nan(2, 4, 2, 2, nTfull);
pCtrl = nan(2, 2, 2, nTfull);
miIS = nan(2, 4, nTfull);
balIS = nan(2, 4, nTfull);
miCal = nan(2, 2, nTfull);
balCal = nan(2, 2, nTfull);
pCal = nan(2, 2, 2, nTfull);
...
for sc = 1:4
    ...
    for iT = 1:nTfull
        ...
        for met = 1:2
            ...
            miMain(met,sc,1,iT) = iMIFromLabels(pB1, yb);
            balMain(met,sc,1,iT) = iBalAcc(pB1, yb);
            ...
            miMain(met,sc,2,iT) = iMIFromLabels(pB2, behTe);
            balMain(met,sc,2,iT) = iBalAcc(pB2, behTe);
            pB1L = 1./(1+exp(-sB1));
            pB2L = 1./(1+exp(-sB2));
            pMain(met,sc,1,1,iT) = mean(pB1L(yb==1));
            pMain(met,sc,1,2,iT) = mean(pB1L(yb==0));
            pMain(met,sc,2,1,iT) = mean(pB2L(behTe==1));
            pMain(met,sc,2,2,iT) = mean(pB2L(behTe==0));
            if sc == 2
                for ci = 1:2
                    ...
                    miCal(met,ci,iT) = iMIFromLabels(pCalT, bcal(okCa));
                    balCal(met,ci,iT) = iBalAcc(pCalT, bcal(okCa));
                    pCalL = 1./(1+exp(-sCal));
                    pCal(met,ci,1,iT) = mean(pCalL(bcal(okCa)==1));
                    pCal(met,ci,2,iT) = mean(pCalL(bcal(okCa)==0));
                end
            end
            if met == 1
            ...
            miIS(met,sc,iT) = iMIFromLabels(pIS, yb);
            balIS(met,sc,iT) = iBalAcc(pIS, yb);
            if sc == 1
                Fc = Ftr(okC,:); yc = cue(okC);
                if sum(yc==1) >= 3 && sum(yc==0) >= 3
                    [sC1, pC1] = iCvPredict(Fc, yc, K, met);
                    miCtrl(met,1,iT) = iMIFromLabels(pC1, yc);
                    balCtrl(met,1,iT) = iBalAcc(pC1, yc);
                    ...
                    miCtrl(met,2,iT) = iMIFromLabels(pC2, r.cueTe);
                    balCtrl(met,2,iT) = iBalAcc(pC2, r.cueTe);
                    pC1A = 1./(1+exp(sC1));
                    pCtrl(met,1,1,iT) = mean(pC1A(yc==0));
                    pCtrl(met,1,2,iT) = mean(pC1A(yc==1));
                    pC2A = 1./(1+exp(sC2));
                    pCtrl(met,2,1,iT) = mean(pC2A(behTe==1));
                    pCtrl(met,2,2,iT) = mean(pC2A(behTe==0));
                end
            end
        end
    end
end
S = struct('miMain',miMain,'balMain',balMain,'miCtrl',miCtrl,'balCtrl',balCtrl, ...
    'pMain',pMain,'pCtrl',pCtrl,'miIS',miIS,'balIS',balIS, ...
    'miCal',miCal,'balCal',balCal,'pCal',pCal);
end

function tbl = iQueryZScoreNts(DS, m, stim)
% Z-scored trial x cell x time table for one mouse and stimulus, built with
% TableQuery (ResampledSignal column) instead of QueryNTS. Rows whose trace is
% all-NaN or all-zero (cells outside the registration range; QueryNTS would
% throw on them) are dropped with a report. Per-row z-score over the full
% window == UniExp.Flags.ZScore with F0Samples = full window (verified).
...
if any(bad)
    nBadTrials = numel(unique(uint64(t.TrialUID(bad))));
    fprintf('    %s %-10s dropped %d bad rows (%d trials affected; cells out of registration)\n', ...
        char(m), stim, sum(bad), nBadTrials);
    sig(bad, :) = [];
    t(bad, :) = [];
end
if height(t) == 0; tbl = table(); return; end
% row-wise z-score over the full trace (== UniExp.Flags.ZScore with F0 = whole
% signal, verified identical to DS.QueryNTS output on a clean mouse)
...
function p = iSignTestP(v, p0)
% one-sided sign test (greater than p0), same statistic as the AL analysis
v = v(~isnan(v));
if isempty(v); p = 1; return; end
k = sum(v > p0); n = numel(v);
p = 1 - binocdf(k-1, n, 0.5);
end