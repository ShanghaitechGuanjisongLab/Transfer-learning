...
weightSvgName = 'TH_Formal_Training_Connection_Polarity_Weight_SD.svg';

Params = iDefaultParams();
Cond = iConditionTable();
workspaceVarNames = iWorkspaceVariableNames();
if iHasReusableWorkspaceSummary(workspaceVarNames)
...
function workspaceVarNames = iWorkspaceVariableNames()
workspaceVarNames.Summary = 'THInhibitoryHeterogeneityModel';
workspaceVarNames.Params = 'THInhibitoryHeterogeneityParams';
workspaceVarNames.Cond = 'THInhibitoryHeterogeneityConditions';
end

function tf = iHasReusableWorkspaceSummary(workspaceVarNames)
...
classification = "source-polarity_abs_output_v2";
end
...
Summary.FormalTrainingConnectionWeightStats = iFormalTrainingConnectionWeightStats(Summary.FormalTrainingConnectionWeights, Cond);
...
Mouse.W_CueInputToL23 = iRandn([Params.NL23, Params.NCueInput], Params) / sqrt(Params.NCueInput);
% Initial reward afferent map into L5 reward-receiving cells.
Mouse.W_RewardToL5RewardRecv = iRandn([Params.NL5RewardRecv, Params.NReward], Params) / sqrt(Params.NReward);
...
sd = Params.InitWStd;
Mouse.W_L23L5ToL23L5 = iZeroSelfProjection(sd * iRandn([Params.NL23L5, Params.NL23L5], Params));
...
Mouse.InhGainL23 = iOnes([Params.NL23, 1], Params);
Mouse.InhGainL5RewardRecv = iOnes([Params.NL5RewardRecv, 1], Params);
...
weightClasses = iAppendConnectionPolarityWeights(weightClasses, -Params.Comp_Cue * (Mouse.InhGainL23 .* Mouse.WEI_L23) / Params.NIL23, -1);
weightClasses = iAppendConnectionPolarityWeights(weightClasses, Mouse.WIE_L5RewardRecv / Params.NL5RewardRecv, 1);
weightClasses = iAppendConnectionPolarityWeights(weightClasses, -Params.Comp_Rew * (Mouse.InhGainL5RewardRecv .* Mouse.WEI_L5RewardRecv) / Params.NIL5RewardRecv, -1);
end
...
for t = 1:NT
	...
	preL5RewardRecv_L = (Mouse.W_RewardToL5RewardRecv * rReward_L) / Params.RewardAfferentNorm + Params.NoiseRew * iRandn([Params.NL5RewardRecv, 1], Params);
	preL5Read_L = Params.ReadInputGain * Mouse.L5ReadoutPattern + Params.NoiseRead * iRandn([Params.NL5Read, 1], Params);
...
function stats = iFormalTrainingConnectionWeightStats(weightValues, Cond)
...
for iClass = 1:numel(classNames)
	...
	for iStage = 1:numel(stageNames)
		...
		for iCond = 1:height(Cond)
			condName = Cond.Name(iCond);
			weights = weightValues.(stageName).(className).(condName);
			[stdMat(iStage, iCond), stdSemMat(iStage, iCond), nMat(iStage, iCond)] = iWeightDistributionStdAndSem(weights);
		end
	end
...
function [stdWeight, stdSem, nWeights] = iWeightDistributionStdAndSem(weights)
weights = weights(:);
...
stdSem = stdWeight / sqrt(2 * (nWeights - 1));
end
...
for iClass = 1:numel(classNames)
	...
	ylabel(ax, 'Weight distribution SD', 'FontSize', 12);
...
for iStage = 1:numel(stageNames)
	...
	for iComparison = 1:size(comparisons, 1)
		...
		weightsA = Summary.FormalTrainingConnectionWeights.(stageName).(className).(Cond.Name(condA));
		weightsB = Summary.FormalTrainingConnectionWeights.(stageName).(className).(Cond.Name(condB));
		pValue = iWeightDispersionPValue(weightsA, weightsB);
...
function pValue = iWeightDispersionPValue(weightsA, weightsB)
weightsA = weightsA(:);
weightsB = weightsB(:);
weightsA = weightsA(isfinite(weightsA));
weightsB = weightsB(isfinite(weightsB));
if numel(weightsA) < 2 || numel(weightsB) < 2
	pValue = NaN;
	return;
end
deviationA = abs(weightsA - median(weightsA, 'omitnan'));
deviationB = abs(weightsB - median(weightsB, 'omitnan'));
pValue = ranksum(deviationA, deviationB);
end
