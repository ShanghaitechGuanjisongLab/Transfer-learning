% Fig3.8C: Hit vs Miss difference in RelDiv(T) within Transfer session (split by layer)
%
% RelDiv computed from per-trial ZScore at 1s in Transfer LightWater,
% computed from the Transfer session alone.
% Paired test: signrank(Hit > Miss)
%
% Execution:
%   TransferLearning.Fig38.C_RelDivTgivenLA_HitVsMiss
...
if isempty(R)
	error('Fig38C:Empty', 'No valid mice for RelDiv(T) Hit/Miss.');
end

layerNames = string(["MOp2/3","MOp5"]);
outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_8c_RelDivTgivenLA_HitVsMiss_2panels.svg";
...
for iZ = 1:numel(layerNames)
	...
	if isfinite(p)
		...
		try
			...
			error('Fig38:C:PLineFailed', 'MATLAB.Graphics.PLine failed:\n%s', getReport(ME, 'extended', 'hyperlinks','off'));
		end
		try
			delete(S);
		catch
