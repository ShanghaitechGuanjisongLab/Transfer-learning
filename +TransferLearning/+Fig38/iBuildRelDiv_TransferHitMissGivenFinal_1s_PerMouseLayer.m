function rows = iBuildRelDiv_TransferHitMissGivenFinal_1s_PerMouseLayer()
% Build per-mouse table for Fig3.8 panels using Transfer relative divergence.
%
% NOTE (naming legacy): earlier drafts used a "|F" suffix to indicate a cell-set
% restriction based on Final. For divergence this is confusing.
%
% Current definition:
%   RelDiv(T): relative divergence of per-trial DeltaF at ~1s in the Transfer
%   LightWater session, computed from the Transfer session alone.
%
% Cohort note:
%   This builder keeps the same *mouse cohort* as the old "T|F" panel by
%   requiring that the mouse has both Transfer and Final LightWater metadata.
%   No Final neural signals are used.
%
% Returns variables (schema kept compatible with plotting scripts):
%   Mouse, Source,
%   DateTimeTransfer, DateTimeFinal (NaT placeholder),
%   TransferHitRate,
...
%
% Execution:
%   TransferLearning.Fig38.iBuildRelDiv_TransferHitMissGivenFinal_1s_PerMouseLayer
...
rows = table;
rows = [rows; iOneDataSet(DS, "AudioLightBaseline", idx1_ref, nT_ref, minTrials, minDenCells)];
