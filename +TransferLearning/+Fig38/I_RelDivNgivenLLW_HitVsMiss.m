% Fig3.8I: Hit vs Miss difference in RelDiv(N) within Naive session (split by layer)
%
% RelDiv(N) computed from per-trial ZScore at 1s in Naive LightWater,
% using Naive session data alone.
% Paired test: signrank(Hit > Miss)
%
% Execution:
%   TransferLearning.Fig38.I_RelDivNgivenLLW_HitVsMiss
...
if isempty(R)
	error('Fig38I:Empty', 'No valid mice for RelDiv(N) Hit/Miss.');
end

layerNames = string(["MOp2/3","MOp5"]);
outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_8i_RelDivNgivenLLW_HitVsMiss_2panels.svg";

f = figure('Color','w', 'Name','RelDiv(N) Hit vs Miss');
MATLAB.Graphics.FigureAspectRatio(1,1,MATLAB.Flags.Narrow);
TL = tiledlayout(1,2,'TileSpacing','compact','Padding','compact');
ylabel(TL, 'RelDiv(N) at 1 s');
...
for iZ = 1:numel(layerNames)
	...
	if isfinite(p)
		...
		try
			...
			error('Fig38:I:PLineFailed', 'MATLAB.Graphics.PLine failed:\n%s', getReport(ME, 'extended', 'hyperlinks','off'));
		end
		try
			delete(S);
		catch
...
sgtitle(TL, 'RelDiv(N) Hit vs Miss (paired)', 'Interpreter','none');
