function pairs = iBuildAdjPairs_DeltaHit_vs_InterSessionCellCorr1p5s()
% Build adjacent-session-pair table (pair = adjacent LightWater DateTimes within mouse):
% - Keep only sessions where calcium exists (QueryNTATS non-empty at 1.5s)
...
%     CellCorr1p5 = corr(NTATS@1.5s vectors across common CellUID)
%
...
if isempty(idx15) || ~isfinite(dtMin) || dtMin > 0.25
	error('Fig37:iBuildAdjPairs:No1p5sSample', 'Cannot find a sample close to 1.5s in TransferLearning.Xs.');
end
...
for iS = 1:numel(spec)
	...
	rows = iOneDataSet(DS, string(S.Source), string(S.Stage), idx15, minCommonCells, requirePureLightWater);
	if ~isempty(rows)
...
function rows = iOneDataSet(DS, sourceName, stageName, idx15, minCommonCells, requirePureLightWater)
	...
	for iM = 1:numel(mice)
		...
		for k = 1:(numel(S)-1)
			A = S(k);
			B = S(k+1);
			[dCorr, nCommon] = iInterSessionCorr(A.Vec, B.Vec, minCommonCells);
			if ~isfinite(dCorr)
				continue;
			end
			dHit = B.Hit - A.Hit;
			rows = [rows; table(m, sourceName, stageName, A.DateTime, B.DateTime, A.Hit, B.Hit, dHit, ...
				nCommon, dCorr, A.NTrials, B.NTrials, 'VariableNames', rows.Properties.VariableNames)]; %#ok<AGROW>
		end
	end
end
...
function [r, nCommon] = iInterSessionCorr(A, B, minCommonCells)
	r = NaN;
	nCommon = NaN;
