% Fig3.7D：放假 7 天组（TransferLearning.Vacation7）的 L-T-F 三泳道热图
%
% 参考：Fig3.6H / Fig3.3A。
...
%
% Execution:
%   TransferLearning.Fig37.D_PTgivenF_vsTransferHitRate

outDirUNC = "\\Data-Server-2\个人数据\张天夫\202601";
svgName = "Fig3_7d_Vacation7_LTFHeatmap_0to2s.svg";
...
if nnz(xMask) < 5
	error('Fig3_7d:BadTimeMask', 'Too few samples in 0~2s window.');
end

baseMask = (xsSec >= -3) & (xsSec < 0);
if nnz(baseMask) < 5
	error('Fig3_7d:BadBaselineMask', 'Too few samples in baseline window (-3~0s).');
end
kSigma = 3;
...
if ~ok1 || ~ok15
	error('Fig3_7d:BadIndex', 'Cannot find sample close to 1s or 1.5s in TransferLearning.Xs.');
end
...
assignin('base','Fig3_7d_Vac7_CellStrip', S);

if ~istable(S) || ~any(strcmp(S.Properties.VariableNames,'CellUID'))
	error('Fig3_7d:NoCellUID', 'Expected CellUID in NtatsCellStrip output.');
end
...
assignin('base','Fig3_7d_Vac7_ActiveMask', activeMask);
assignin('base','Fig3_7d_Vac7_ActiveByLane_1sOr1p5', activeByLane);
assignin('base','Fig3_7d_Vac7_ActiveCellUID', uidAll(activeMask));

XActive = X(activeMask, :, :);
zKeyActive = zKeyAll(activeMask);

% --- 6) Split by layer and sort within each layer
mask23 = (zKeyActive == "MOp23");
mask5  = (zKeyActive == "MOp5");

[laneData23, sortIdx23, delta23] = iBuildLaneDataPerLayer(XActive, mask23, xMask, idx1s);
[laneData5,  sortIdx5,  delta5 ] = iBuildLaneDataPerLayer(XActive, mask5,  xMask, idx1s);

assignin('base','Fig3_7d_Vac7_Mask_MOp23', mask23);
assignin('base','Fig3_7d_Vac7_Mask_MOp5', mask5);
assignin('base','Fig3_7d_Vac7_SortIdx_MOp23', sortIdx23);
assignin('base','Fig3_7d_Vac7_SortIdx_MOp5', sortIdx5);
assignin('base','Fig3_7d_Vac7_Delta1s_MOp23', delta23);
assignin('base','Fig3_7d_Vac7_Delta1s_MOp5', delta5);

% --- 8) Plot (2 rows x 3 cols)
f = figure('Color','w', 'Name', 'Fig3.7D Vacation7 L-T-F heatmap by layer (0~2s)');
try
...
% CLim: symmetric colormap with sqrt-compressed range (per request)
mag0 = max(abs([negV, posV]));
if ~isfinite(mag0)
	mag0 = 1;
end
mag = sqrt(max(mag0, 0));
...
assignin('base','Fig3_7d_Vac7_CLim', CLim);
...
if isempty(T)
	error('Fig3_7d:NoTransferLW', 'No Transfer LightWater trials found.');
end
...
if ~isempty(badEmpty)
	error('Fig3_7d:TransferLWBadDateTime', 'Some mice have Transfer LW trials but missing DateTime: %s', strjoin(badEmpty, ', '));
end
if ~isempty(badMulti)
	error('Fig3_7d:TransferLWMulitpleSessions', 'Transfer LightWater is not a single session for mice: %s', strjoin(badMulti, ', '));
end
if ~isempty(badMix)
	error('Fig3_7d:TransferLWMixedStim', 'Transfer LightWater session contains AudioWater trials for mice: %s', strjoin(unique(badMix), ', '));
end
end
...
elseif isnumeric(nt)
	...
	error('Fig3_7d:BadNTATS', 'Unsupported NTATS container type: %s', class(nt));
end

Xall = squeeze(double(Xall));
if ndims(Xall) ~= 3
	error('Fig3_7d:BadNTATSDims', 'Expected NTATS to be 3-D after squeeze.');
end
