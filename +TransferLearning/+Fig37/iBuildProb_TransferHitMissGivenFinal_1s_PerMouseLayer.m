function rows = iBuildProb_TransferHitMissGivenFinal_1s_PerMouseLayer(varargin)
% Build per-mouse table for Fig3.7 panels involving P(T|F) and Hit/Miss variants.
%
% Definitions (per mouse, per layer):
%   F = Final LightWater (last pure session; forbidden stimulus=AudioWater)
...
%
% P(T|F) computed over common cells between chosen sessions.
% Also computes within the chosen Transfer session:
%   P(T_hit|F), P(T_miss|F)
%
% Returns variables:
%   Mouse, Source,
%   DateTimeTransfer, DateTimeFinal,
%   TransferHitRate,
%   NCommonCells,
%   NFinalActive23, NFinalActive5,
%   Prob23, Prob5,
...
ip = inputParser;
ip.FunctionName = 'TransferLearning.Fig37.iBuildProb_TransferHitMissGivenFinal_1s_PerMouseLayer';
ip.addParameter('DataSet', [], @(x) isempty(x) || isobject(x));
ip.addParameter('Source', "AudioLightBaseline", @(x) isstring(x) || ischar(x));
ip.parse(varargin{:});
...
if ~any(baseMask)
	error('Fig37:iBuildPTgivenF:BadTimeMask', 'Baseline(-3~0) has no samples.');
end
idx1 = find(xsSec == 1, 1, 'first');
if isempty(idx1)
	[dtMin, idx1] = min(abs(xsSec - 1));
	if isempty(idx1) || ~isfinite(dtMin) || dtMin > 0.25
		error('Fig37:iBuildPTgivenF:No1sSample', 'Cannot find a sample close to 1s in TransferLearning.Xs.');
	end
end
...
function out = iOneDataSet(DS, sourceName, baseMask, idx1, kSigma, minTrials, minCommonCells)
	out = table(string.empty(0,1), string.empty(0,1), NaT(0,1), NaT(0,1), ...
		...
		'VariableNames', {'Mouse','Source','DateTimeTransfer','DateTimeFinal', ...
		'TransferHitRate','NCommonCells','NFinalActive23','NFinalActive5', ...
		'Prob23','Prob5','ProbHit23','ProbMiss23','ProbHit5','ProbMiss5'});

	miceT = iMiceInPhaseStimulus(DS, "Transfer", "LightWater");
	miceF = iMiceInPhaseStimulus(DS, "Final",    "LightWater");
	mice = intersect(miceT, miceF);
	...
	for iM = 1:numel(mice)
		...
		[Tf, dtF] = iTrialsByMousePureSession(DS, m, "Final",    "LightWater", "last",  forbiddenStimulus="AudioWater");
		if isempty(Tt) || isempty(Tf) || numel(Tt) < minTrials || numel(Tf) < minTrials
			continue;
		end
		...
		Zf = iMedianTraceZScore(DS, cellUID, Tf);
		if isempty(Zt) || isempty(Zf)
			continue;
		end

		uidCommon = intersect(uint64(Zt.CellUID), uint64(Zf.CellUID));
		if numel(uidCommon) < minCommonCells
			continue;
		end

		ZtC = sortrows(Zt(ismember(uint64(Zt.CellUID), uidCommon), :), 'CellUID');
		ZfC = sortrows(Zf(ismember(uint64(Zf.CellUID), uidCommon), :), 'CellUID');
		uidCommon = uint64(ZtC.CellUID);

		actT = iIsActive(ZtC.Trace, baseMask, idx1, kSigma);
		actF = iIsActive(ZfC.Trace, baseMask, idx1, kSigma);

		zl = iCellZLayer(DS, uidCommon);
		m23 = (zl == "MOp2/3");
		m5  = (zl == "MOp5");

		[nF23, p23] = iProbAlsoActive(actT, actF, m23);
		[nF5,  p5]  = iProbAlsoActive(actT, actF, m5);
		...
		if ~isempty(Thit)
			...
			if ~isempty(Zh)
				...
				[~, probHit23] = iProbAlsoActive(actH, actF, m23);
				[~, probHit5]  = iProbAlsoActive(actH, actF, m5);
			end
		end
		if ~isempty(Tmiss)
			...
			if ~isempty(Zm)
				...
				[~, probMiss23] = iProbAlsoActive(actM, actF, m23);
				[~, probMiss5]  = iProbAlsoActive(actM, actF, m5);
			end
		end

		out = [out; table(m, string(sourceName), dtT, dtF, perfT, numel(uidCommon), nF23, nF5, p23, p5, ...
