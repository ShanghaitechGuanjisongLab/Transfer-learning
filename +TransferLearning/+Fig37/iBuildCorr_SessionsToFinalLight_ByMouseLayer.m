function Out = iBuildCorr_SessionsToFinalLight_ByMouseLayer(DS, groupName, varargin)
% Build session-level Corr(Session LightWater, Correct Final LightWater) per mouse/layer.
%
% Correct session definition:
% - Phase="Final", Stimulus="LightWater"; choose last DateTime
% - Must be pure (forbidden stimulus default AudioWater)
%
% Session pool definition (Fig3.4F-style):
% - LightWater sessions from first StartPhase to last EndPhase (inclusive)
% - Exclude sessions with forbidden stimulus (default AudioWater)
...
% - Exclude the correct session itself (by default)
%
% Output variables:
%   Group, Mouse, ZKey, DateTimeCorrect, DateTimeSession, NCommon, Corr, FisherZ, HasData

ip = inputParser;
ip.FunctionName = 'TransferLearning.Fig37.iBuildCorr_SessionsToFinalLight_ByMouseLayer';
...
addParameter(ip, 'Stimulus', "LightWater", @(s) isstring(s) || ischar(s));
addParameter(ip, 'CorrectPhase', "Final", @(s) isstring(s) || ischar(s));
addParameter(ip, 'ForbiddenStimulus', "AudioWater", @(s) isstring(s) || ischar(s));
addParameter(ip, 'ApplyAfter100', true, @(x) islogical(x) && isscalar(x));
addParameter(ip, 'ExcludeCorrectSessionItself', true, @(x) islogical(x) && isscalar(x));
parse(ip, varargin{:});
...
stimulusName = string(ip.Results.Stimulus);
correctPhase = string(ip.Results.CorrectPhase);
forbidden = string(ip.Results.ForbiddenStimulus);
applyAfter100 = logical(ip.Results.ApplyAfter100);
excludeCorrect = logical(ip.Results.ExcludeCorrectSessionItself);

groupName = string(groupName);

Out = table(string.empty(0,1), string.empty(0,1), string.empty(0,1), NaT(0,1), NaT(0,1), nan(0,1), nan(0,1), nan(0,1), nan(0,1), ...
	'VariableNames', {'Group','Mouse','ZKey','DateTimeCorrect','DateTimeSession','NCommon','Corr','FisherZ','HasData'});
...
if isempty(idxT) || ~isfinite(dtMin) || dtMin > 0.25
	error('Fig37:CorrToFinal:NoTargetSample', 'Cannot find a sample close to %.3gs in TransferLearning.Xs.', targetAtSec);
end
...
% Candidate mice: must have correct phase + stimulus
mice = iMiceInPhaseStimulus(DS, correctPhase, stimulusName);
if isempty(mice)
	return;
end
...
for iM = 1:numel(mice)
	...
	% Correct session: last pure Final LightWater
	[Tc, dtC] = iTrialsByMousePureSession(DS, mouse, correctPhase, stimulusName, "last", forbiddenStimulus=forbidden);
	if isempty(Tc) || isnat(dtC)
		continue;
	end
	[uidC, vC] = iSessionVecByTrialUID(DS, cellAll, Tc, idxT);
	if isempty(uidC) || isempty(vC)
		continue;
	end

	% Session pool: LightWater from startPhase..endPhase
	Sess = iSessionPoolPhaseRange(DS, mouse, stimulusName, startPhase, endPhase);
	if isempty(Sess)
		continue;
	end
	...
	for k = 1:height(Sess)
		...
		if excludeCorrect && ~isnat(dtC) && dt == dtC
			continue;
		end
		if iHasStimulus(DS, mouse, dt, forbidden)
			continue;
		end
		hit = iHitRateFromTrialUID(DS, trialUID, stimulusName);
		if ~isfinite(hit)
			continue;
		end
		[uidS, vS] = iSessionVecByTrialUID(DS, cellAll, trialUID, idxT);
		if isempty(uidS) || isempty(vS)
			continue;
		end
		usable(end+1) = struct('DateTime', dt, 'Hit', double(hit), 'CellUID', uint64(uidS), 'Val', double(vS)); %#ok<AGROW>
	end
	if numel(usable) < 1
		continue;
	end
	...
	zC = iCellZKey(Cm, uidC);
	for k = 1:numel(usable)
		dtS = usable(k).DateTime;
		uidS = usable(k).CellUID;
		vS = usable(k).Val;
		zS = iCellZKey(Cm, uidS);
		...
		for zKey = ["MOp23","MOp5"]
			...
			[r, n] = iPearsonOnCommon(uidSz, vSz, uidCz, vCz, minCommonCells);
			...
			Out = [Out; table(groupName, mouse, string(zKey), dtC, dtS, double(n), double(r), double(z), 1, ...
...
function Sess = iSessionPoolPhaseRange(DS, mouseName, stimName, startPhase, endPhase)
