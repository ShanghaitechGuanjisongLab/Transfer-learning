% Fig3.7J (new): Adjacent-session DeltaHit vs inter-session CellCorr at 1.5s
%
...
% - CellCorr1p5 = corr(NTATS@1.5s vectors across common cells)
%
% Then test whether DeltaHit positively correlates with CellCorr1p5 (pair as unit).
%
% Execution:
%   TransferLearning.Fig37.J_DeltaHit_vs_InterSessionCellCorr1p5s
...
pairs = TransferLearning.Fig37.iBuildAdjPairs_DeltaHit_vs_InterSessionCellCorr1p5s();
if isempty(pairs)
	error('Fig37J:Empty', 'No valid adjacent session pairs.');
end

outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_7j_DeltaHit_vs_InterSessionCellCorr1p5s.svg";

f = figure('Color','w', 'Name','DeltaHit vs inter-session CellCorr@1.5s');
...
for iS = 1:2
	...
	grid(ax,'on');
	box(ax,'off');
	xlabel(ax, 'Inter-session CellCorr @1.5s');
...
sgtitle(TL, 'Adjacent-session behavior change vs NTATS inter-session similarity (1.5s)', 'Interpreter','none');
...
% Also print pooled stats
maskAll = isfinite(pairs.CellCorr1p5) & isfinite(pairs.DeltaHit);
if nnz(maskAll) >= 6 && std(pairs.CellCorr1p5(maskAll)) > 0 && std(pairs.DeltaHit(maskAll)) > 0
	[rS, pS] = corr(pairs.CellCorr1p5(maskAll), pairs.DeltaHit(maskAll), 'Type','Spearman');
	[rP, pP] = corr(pairs.CellCorr1p5(maskAll), pairs.DeltaHit(maskAll), 'Type','Pearson');
