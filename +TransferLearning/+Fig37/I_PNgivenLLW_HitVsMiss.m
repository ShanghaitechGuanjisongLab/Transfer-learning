% Fig3.7I: Hit vs Miss difference in P(N|L) within Naive session (split by layer)
%
% P(N_hit|L) and P(N_miss|L) computed using:
%   L = Learned LightWater active at 1s (last pure session)
%   N_hit/N_miss = Naive LightWater Hit/Miss active at 1s within first pure session
% Paired test: signrank(Hit > Miss)
%
% Execution:
%   TransferLearning.Fig37.I_PNgivenLLW_HitVsMiss
...
if isempty(R)
	error('Fig37I:Empty', 'No valid mice for P(N|L) Hit/Miss.');
end

layerNames = string(["MOp2/3","MOp5"]);
outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_7i_PNgivenLLW_HitVsMiss_2panels.svg";

f = figure('Color','w', 'Name','P(N|L) Hit vs Miss');
...
for iZ = 1:numel(layerNames)
	...
	if isfinite(p)
		...
		try
			...
			error('Fig37:I:PLineFailed', 'MATLAB.Graphics.PLine failed: %s', ME.message);
		end
		try
			delete(S);
		catch
