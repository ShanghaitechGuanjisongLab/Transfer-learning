% Fig3.7J (by-layer): Adjacent-session DeltaHit vs inter-session CellCorr at 1.5s
%
% Same as J_DeltaHit_vs_InterSessionCellCorr1p5s, but split by layer (MOp2/3 vs MOp5).
%
% Execution:
%   TransferLearning.Fig37.J_DeltaHit_vs_InterSessionCellCorr1p5s_ByLayer
...
if isempty(pairs)
	error('Fig37JCellCorrByLayer:Empty', 'No valid adjacent session pairs (by layer).');
end

outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_7j_DeltaHit_vs_InterSessionCellCorr1p5s_ByLayer.svg";

f = figure('Color','w', 'Name','DeltaHit vs inter-session CellCorr@1.5s (by layer)');
...
sgtitle(TL, 'Adjacent-session behavior change vs NTATS inter-session similarity (1.5s, by layer)', 'Interpreter','none');
...
for zKey = layerList
	maskAll = isfinite(pairs.CellCorr1p5) & isfinite(pairs.DeltaHit) & (pairs.ZKey == zKey);
