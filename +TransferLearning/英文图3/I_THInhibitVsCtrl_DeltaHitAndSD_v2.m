...
% --- Per-group data: session-pair ΔHit, session-level SD
[dhC, sdC] = iCohortData(CtrlDS, idx1s, "Transfer", "Final");
[dhT, sdT] = iCohortData(THDS,   idx1s, "Transfer", "Final");

fprintf('Ctrl: %d pairs (ΔHit), %d mice (SD)\n', numel(dhC), numel(sdC));
fprintf('TH:   %d pairs (ΔHit), %d mice (SD)\n', numel(dhT), numel(sdT));
...
f = figure('Color', 'w', 'Name', 'Fig3I TH ΔHit & Heterogeneity');
f.Units = 'centimeters';
f.Position(3:4) = [3, 4];
...
[~, Opt1, Bars1, EB1] = UniExp.BarScatterCompare( ...
	{dhC, dhT}, false, CompareGroup, 'AsteriskThreshold', 0.05);
...
[~, Opt2, Bars2, EB2] = UniExp.BarScatterCompare( ...
	{sdC, sdT}, false, CompareGroup, 'AsteriskThreshold', 0.05);
...
%   sdVec: Response heterogeneity per mouse (avg-first, one element per mouse)
...
% --- Session-pair ΔHit (ceiling-excluded) + track per-mouse sessions ---
...
% --- Per-mouse SD (avg-first: per-cell-per-session median → per-cell mean → filter → std) ---
