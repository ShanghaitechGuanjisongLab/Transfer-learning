% Fig3.3E: Hit vs Miss difference in P(T|F) within Transfer session (split by layer)
%
% Paired test: signrank(Hit > Miss)
% Builder: TransferLearning.Fig37.iBuildProb_TransferHitMissGivenFinal_1s_PerMouseLayer
%
% Execution:
%   TransferLearning.Fig33.E_PTgivenF_HitVsMiss
...
if isempty(R)
	error('Fig33E:Empty', 'No valid mice for P(T|F) Hit/Miss.');
end

layerNames = string(["MOp2/3","MOp5"]);
outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_3e_PTgivenF_HitVsMiss_2panels.svg";

f = figure('Color','w', 'Name','Fig3.3E P(T|F) Hit vs Miss');
...
for iZ = 1:numel(layerNames)
	...
	if isfinite(p)
		...
		try
			...
			error('Fig33:E:PLineFailed', 'MATLAB.Graphics.PLine failed: %s', ME.message);
		end
		try
			delete(S);
		catch
...
sgtitle(TL, 'P(T|F) Hit vs Miss (paired)', 'Interpreter','none');
