% Fig3.3H: Difference between P(T|F) and P(N|L) (UNPAIRED cohorts, split by layer)
%
% NOTE: P(T|F) cohort and P(N|L) cohort are NOT the same mice.
% Statistics: ranksum (Mann–Whitney U) two-sided per layer.
%
% Execution:
%   TransferLearning.Fig33.H_PNgivenLLW_vs_PTgivenF_Difference
...
if isempty(RT) || isempty(RN)
	error('Fig33H:Empty', 'Empty builder output. RT empty=%d, RN empty=%d.', isempty(RT), isempty(RN));
end

layerNames = string(["MOp2/3","MOp5"]);
outDir = iSelectOutDir(outDirUNC);
svgName = "Fig3_3h_PNgivenLLW_vs_PTgivenF_UnpairedDifference_2panels.svg";

f = figure('Color','w', 'Name','Fig3.3H P(T|F) vs P(N|L) (unpaired)');
...
for iZ = 1:numel(layerNames)
	...
	if isfinite(p)
		...
		try
			...
			error('Fig33:H:PLineFailed', 'MATLAB.Graphics.PLine failed: %s', ME.message);
		end
		try
			delete(S);
		catch
