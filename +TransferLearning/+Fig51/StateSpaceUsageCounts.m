...
for groupIndex = 1:numel(representativeGroups)
	...
	T = [T; table("C", representativeGroups(groupIndex), representativeGroups(groupIndex), string(rep.Mouse), string(rep.Source), 1, numel(mouseStates(stateIndex).CellUID), ...
...
for layerIndex = 1:numel(layerNames)
	...
	for groupIndex = 1:numel(groupNames)
		...
		T = [T; table("D-F", layerName, groupName, groupName, nMouse, nCell, 'VariableNames', T.Properties.VariableNames)]; %#ok<AGROW>
	end
end
end
...
if isempty(stateIndex)
	error('Fig51:MissingMouseStateForCounts', 'Cannot find MouseStates row for mouse %s, source %s.', string(mouseName), string(sourceName));
end
end
