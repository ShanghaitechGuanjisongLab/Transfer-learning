% English Fig3A: Reuse rate (1s) vs Hit-rate increment (ΔHit), layers merged (L2/3 + L5)
%
...
%   TransferLearning.EnglishFig3.A_ReuseRateVsDeltaHit
...
for i = 1:height(SessKey)
	...
	% Use NTS to avoid QueryNTATS empty-group errors for some sessions.
...
% nts table must contain CellUID and TrialSignal (nTrial x nTime).
...
for iC = 1:numel(cellUIDs)
	...
	sig = double(nts.TrialSignal(rows, :)); % [nTrial x nTime]
	if isempty(sig) || ~ismatrix(sig)
		continue;
	end
	med = median(sig, 1, 'omitnan'); % [1 x nTime]
