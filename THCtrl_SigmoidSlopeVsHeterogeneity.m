% THCtrl sigmoid slope vs heterogeneity using Fig1B-style sigmoid slope method

outDirUNC = '\\Data-Server-2\个人数据\杨青宁\202605';
svgName = 'THCtrl_SigmoidSlopeVsHeterogeneity.svg';
scriptCopyName = 'THCtrl_SigmoidSlopeVsHeterogeneity.m';
dataCsvName = 'THCtrl_SigmoidSlopeVsHeterogeneity_Data.csv';
statsCsvName = 'THCtrl_SigmoidSlopeVsHeterogeneity_Stats.csv';
...
if ~ok1s
	error('THCtrl_SigmoidSlopeVsHeterogeneity:No1s', 'Cannot find sample close to 1s in time axis.');
end
...
f = figure('Color', 'w', 'Name', 'THCtrl sigmoid slope vs heterogeneity');
f.Units = 'centimeters';
f.Position(3:4) = [12, 8];
...
for iL = 1:numel(layers)
	...
	fprintf('\n=== THCtrl sigmoid slope vs heterogeneity %s ===\n', layerLabels(iL));
	fprintf('Ctrl mice: %d\n', nnz(maskC));
	fprintf('TH mice: %d\n', nnz(maskT));
...
assignin('base', 'THCtrl_SigmoidSlopeVsHeterogeneity_Data', dataTable);
assignin('base', 'THCtrl_SigmoidSlopeVsHeterogeneity_Stats', statsRows);
...
function fitOut = iFitSigmoidCurve(T, groupName)
	...
	yObs = yObs(use);
	if isempty(xObs)
		error('THCtrl_SigmoidSlopeVsHeterogeneity:NoDataForGroup', 'No valid session data for group %s.', char(groupName));
	end
