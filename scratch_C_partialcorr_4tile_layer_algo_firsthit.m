% 临时探索：2/5层 × 两种异质性算法 的 partial correlation
% 控制变量：用于拟合学习斜率的首会话命中率 FirstHit
% 相关对象：Response heterogeneity vs Learning slope
...
if ~ok1s
    error('scratch_C_partialcorr_4tile_layer_algo_firsthit:No1s', 'Cannot find sample close to 1s in time axis.');
end

layers = ["MOp2/3"; "MOp5"];
layerLabels = ["L2/3"; "L5"];
algorithms = ["Current"; "TrialMean"];
algorithmLabels = ["Current algorithm"; "Trial SD then mouse mean"];

CellLAB = iCellLayerTable(DS_LAB, "LAB");
CellLAI = iCellLayerTable(DS_LAI, "LAI");
CellTransfer = iCellLayerTable(DS_Transfer, "Transfer");

summaryRows = table('Size', [0 11], ...
    'VariableTypes', {'string','string','double','double','double','double','double','double','double','double','double'}, ...
    'VariableNames', {'Layer','Algorithm','NaiveN','TransferN','N','PartialRho','PartialP','RawRho','RawP','FirstHitRho','FirstHitP'});

f = figure('Color', 'w', 'Name', 'Fig3C partial correlation by layer and algorithm');
f.Units = 'centimeters';
f.Position(3:4) = [12, 8];
...
for iA = 1:numel(algorithms)
    ...
    for iL = 1:numel(layers)
        ...
        [slopeN, sdN, firstHitN, miceN] = iNaiveCohortData(DS_LAB, DS_LAI, CellLAB, CellLAI, idx1s, layerName, algorithmName);
        [slopeT, sdT, firstHitT, miceT] = iTransferCohortData(DS_Transfer, CellTransfer, idx1s, layerName, algorithmName);
        ...
        if nnz(use) >= 3 && std(sdAll(use)) > 0 && std(slopeAll(use)) > 0
            [rawRho, rawP] = corr(sdAll(use), slopeAll(use), 'Type', 'Spearman');
        else
            rawRho = NaN; rawP = NaN;
        end

        if nnz(use) >= 3 && std(firstHitAll(use)) > 0 && std(sdAll(use)) > 0
            [firstHitRho, firstHitP] = corr(firstHitAll(use), sdAll(use), 'Type', 'Spearman');
        else
            firstHitRho = NaN; firstHitP = NaN;
        end

        if nnz(use) >= 4 && std(sdAll(use)) > 0 && std(slopeAll(use)) > 0 && std(firstHitAll(use)) > 0
            [partialRho, partialP] = partialcorr(sdAll(use), slopeAll(use), firstHitAll(use), 'Type', 'Spearman');
        else
            partialRho = NaN; partialP = NaN;
        end

        row = table(layerLabel, algorithmLabel, numel(miceN), numel(miceT), nnz(use), ...
            partialRho, partialP, rawRho, rawP, firstHitRho, firstHitP, ...
        ...
        fprintf('Raw Spearman: rho = %.6f, p = %.6g\n', rawRho, rawP);
        fprintf('Partial Spearman | FirstHit: rho = %.6f, p = %.6g\n', partialRho, partialP);
        fprintf('FirstHit vs heterogeneity: rho = %.6f, p = %.6g\n', firstHitRho, firstHitP);
        ...
        text(ax, 0.97, 0.97, sprintf('partial rho=%.3f\np=%s\nn=%d', partialRho, iPString(partialP), nnz(use)), ...
...
pngUNC = fullfile(outDirUNC, 'English_Fig3C_4Tile_PartialCorr_LayerAlgorithm_FirstHit.png');
pngLocal = fullfile(outDirLocal, 'English_Fig3C_4Tile_PartialCorr_LayerAlgorithm_FirstHit.png');
...
function [slopeVec, sdVec, firstHitVec, miceKept] = iNaiveCohortData(DS_LAB, DS_LAI, CellLAB, CellLAI, idx1s, layerName, algorithmName)
...
function [slopeVec, sdVec, firstHitVec, miceKept] = iTransferCohortData(DS, CellMap, idx1s, layerName, algorithmName)
