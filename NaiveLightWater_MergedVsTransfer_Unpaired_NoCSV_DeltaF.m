%% Merge Naive (LAInterspersed v5 + LightAudioBaseline) vs Transfer (AudioLightBaseline) — DeltaF
% 不依赖 CSV 回读：直接从数据库对象计算 divergence 并做非配对 ranksum。
%
% Normalize: UniExp.Flags.DeltaF
% Baseline indices: 1:24 (fixed)
%
% Divergence (new definition):
%   numerator = sqrt(mean_c( var_t( Z(c,t) ) ))
%   denominator = norm( mean_t(Z(:,t)) )
% where Z is cell×trial matrix at 1s sample.
...
assignin('base','NaiveMergedVsTransfer_NoCSV_DeltaF_Summary', summary);
assignin('base','NaiveMerged_NoCSV_DeltaF_Rows', rowsNaive);
assignin('base','Transfer_NoCSV_DeltaF_Rows', rowsTransfer);

fprintf("Merged Naive vs Transfer (DeltaF, unpaired ranksum): p=%.6g (Naive n=%d, Transfer n=%d)\n", p, numel(x), numel(y));

%% --- 5) Export (results only)
iWriteTableUNC(summary, outDirUNC, 'NaiveMergedVsTransfer_Divergence_Unpaired_NoCSV_DeltaF.csv');
...
out = table(string.empty(0,1), string.empty(0,1), nan(0,1), nan(0,1), nan(0,1), nan(0,1), nan(0,1), ...
    'VariableNames', {'DataSet','Mouse','NCells','NTrials','StdCellTrial','CentroidToOrigin','Divergence'});
if isempty(mice)
    return;
end
...
for iM = 1:numel(mice)
    ...
    ntsCell = DS.QueryNTS(struct('Stimulus','LightWater','Mouse',m), UniExp.Flags.DeltaF, 1:24);
    nts = ntsCell{1};
    ...
    out = [out; table(string(dsName), string(m), nCell, nTrial, stdCellTrial, dist0, div, ...
        'VariableNames', {'DataSet','Mouse','NCells','NTrials','StdCellTrial','CentroidToOrigin','Divergence'})]; %#ok<AGROW>
end
end
...
out = table(string.empty(0,1), nan(0,1), nan(0,1), nan(0,1), nan(0,1), nan(0,1), ...
    'VariableNames', {'Mouse','NCells','NTrials','StdCellTrial','CentroidToOrigin','Divergence'});
...
for iM = 1:numel(allMice)
    ...
    ntsCell = DS.QueryNTS(struct('Stimulus','LightWater','Design','LightWater','Mouse',m), UniExp.Flags.DeltaF, 1:24);
    nts = ntsCell{1};
    ...
    out = [out; table(string(m), nCell, nTrial, stdCellTrial, dist0, div, ...
        'VariableNames', {'Mouse','NCells','NTrials','StdCellTrial','CentroidToOrigin','Divergence'})]; %#ok<AGROW>
end
end
