...
x = sdAll(use);
y = slopeAll(use);
mouseUse = mouseAll(use);
groupUse = groupAll(use);
...
Tout = table(mouseUse, groupUse, x, y, yHat, resid, absResid, ...
	'VariableNames', {'Mouse','Group','Heterogeneity','Slope','FitSlope','Residual','AbsResidual'});
Tout = sortrows(Tout, 'AbsResidual', 'descend');

fprintf('Top outliers by absolute residual from displayed linear fit:\n');
