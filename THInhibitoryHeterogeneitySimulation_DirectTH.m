...
CheckpointTag = 'direct_th_mid100_slope8_cuegain50_prehebb600_formhebb310_state030_wcap500_inh10_th220_thnoise010_thr060_noise014_iter006_elig085_ret094_initscale082_fixedcue_precue014_cue018_ov000_i12'; % change this to keep separate checkpoint lines.
...
f.Units = 'centimeters';
f.Position(3:4) = [18, 5.5];
f.PaperUnits = 'centimeters';
f.PaperPositionMode = 'manual';
f.PaperPosition = [0, 0, 18, 5.5];
f.PaperSize = [18, 5.5];
...
% Formal training stops per mouse after the first 100%-hit unit; heterogeneity
% summaries average all units that were actually trained.
...
Params.PreCueL23ActiveFraction = 0.14;
Params.CueL23ActiveFraction = 0.18;
Params.CueL23OverlapFractionOfPreCue = 0.00;
...
for iSess = RunState.CompletedFormalSessions + 1:nSess
	completedBefore = iFormalTrainingCompletedMask(perfAll, iSess - 1, Params);
	taskShouldTrain = ~reshape(completedBefore, nTask, 1);
	fprintf('Starting formal session %d/%d: active=%d/%d.\n', iSess, nSess, nnz(taskShouldTrain), nTask);
	mouseCells = MousePool(:);
	...
	parfor iTask = 1:nTask
		...
		if taskShouldTrain(iTask)
			...
			perfTask = Params.Ceiling;
		end
		perfSession(iTask) = perfTask;
		h23Session(iTask) = iTrainingUnitMeanRestrictedStd(sessionMeanL23Task);
		h5Session(iTask) = iTrainingUnitMeanRestrictedStd(sessionMeanL5Task);
	...
	if iSess < nSess
		completedNow = iFormalTrainingCompletedMask(perfAll, iSess, Params);
		taskShouldConsolidate = ~reshape(completedNow, nTask, 1);
		mouseCells = MousePool(:);
		parfor iTask = 1:nTask
			if taskShouldConsolidate(iTask)
...
iCheckTransferSignificantlyHighest(Summary.PerMouse, Cond, Params);
end

function completedMask = iFormalTrainingCompletedMask(perfAll, lastSess, Params)
completedMask = false(size(perfAll, 1), size(perfAll, 2));
if lastSess < 1
	return;
end
completedMask = any(perfAll(:, :, 1:lastSess) >= Params.SlopeHitPerfect, 3);
end
...
function iCheckTransferSignificantlyHighest(PerMouse, Cond, Params)
metricNames = ["MeanH5"; "Slope"];
metricLabels = ["Mean L5 heterogeneity"; "Learning slope"];
for iMetric = 1:numel(metricNames)
	iCheckTransferMetricSignificantlyHighest(PerMouse, Cond, metricNames(iMetric), metricLabels(iMetric), Params.TransferHighestAlpha);
end
end
...
for iCond = 1:height(Cond)
	...
	if isempty(transferValues) || isempty(otherValues)
		failedComparisons(end + 1, 1) = sprintf('%s: finite samples Transfer=%d, %s=%d', ...
			Cond.Name(iCond), numel(transferValues), Cond.Label(iCond), numel(otherValues)); %#ok<AGROW>
		continue;
	end
...
useIdx = iTrainingUnitIndex(sessionMeanL23, sessionMeanL5);
if ~isempty(useIdx)
	if numel(useIdx) >= 2
		fitY = perf(useIdx)';
		dh = diff(fitY);
...
for iSess = 1:Params.NumSessions
	...
	h23(iSess) = iTrainingUnitMeanRestrictedStd(sessionMeanL23);
	h5(iSess)  = iTrainingUnitMeanRestrictedStd(sessionMeanL5);

	if perf(iSess) >= Params.SlopeHitPerfect
		if iSess < Params.NumSessions
			perf(iSess + 1:end) = Params.Ceiling;
			h23(iSess + 1:end) = h23(iSess);
			h5(iSess + 1:end) = h5(iSess);
		end
		break;
	end
...
function useIdx = iTrainingUnitIndex(sessionMeanL23, sessionMeanL5)
l23HasTrainingUnit = any(isfinite(sessionMeanL23), 1);
l5HasTrainingUnit = any(isfinite(sessionMeanL5), 1);
useIdx = find(l23HasTrainingUnit & l5HasTrainingUnit);
end

function metricValue = iTrainingUnitMeanRestrictedStd(sessionMean)
useIdx = find(any(isfinite(sessionMean), 1));
if isempty(useIdx)
	metricValue = NaN;
else
	metricValue = iRestrictedStd(mean(sessionMean(:, useIdx), 2, 'omitnan'));
end
end
