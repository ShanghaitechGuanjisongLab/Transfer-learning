% FigS1C (English, from Chinese Fig32E): trial-wise hit rate within the first light-water block (Naive vs Transfer), curve only
...
if isempty(allSessions)
	error('FigS1C:EmptyData', 'No LightWater sessions found.');
end
...
if isempty(trialRows)
	error('FigS1C:EmptyTrials', 'No first training-unit LightWater trials found.');
end
...
f = figure('Color','w', 'Name', 'FigS1C undelayed first-block trial curve');
f.Units = 'centimeters';
f.Position(3:4) = [9, 8];
ax = axes(f);
ax.FontSize = 12;
...
svgPath = TransferLearning.ExportStandardFigure(f, 2, 'English_FigS1C_UndelayedFirstBlockTrialCurve.svg');
fprintf('Wrote: %s\n', svgPath);
...
assignin('base', 'FigS1C_FirstBlockTrial_Raw', trialRows);
assignin('base', 'FigS1C_FirstBlockTrial_Summary', summaryCurve);
...
dup = mice(nSrc > 1);
if ~isempty(dup)
	error('FigS1C:DuplicateMouseAcrossSources', 'Group %s has duplicated mice across sources.', char(string(groupName)));
end
end
...
dup = mice(nG > 1);
if ~isempty(dup)
	error('FigS1C:MouseInMultipleGroups', 'Some mice appear in multiple groups.');
end
end
