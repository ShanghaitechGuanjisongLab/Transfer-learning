...
%   下方: F1 按🔊💧正/负分组的💡💧 z-score (对应E的x轴分割)
%   左侧: F2 按💡💧正/负分组的🔊💧 z-score (对应E的y轴分割, 水平条形图)
%
...
fprintf('Bottom (split by AW sign): AW+ LW=%.4f+/-%.4f, AW- LW=%.4f+/-%.4f, p=%.4g\n', ...
	mean(meanLW_AWpos(vPN1)), std(meanLW_AWpos(vPN1))/sqrt(sum(vPN1)), ...
	mean(meanLW_AWneg(vPN1)), std(meanLW_AWneg(vPN1))/sqrt(sum(vPN1)), pPN1);
fprintf('Left   (split by LW sign): LW+ AW=%.4f+/-%.4f, LW- AW=%.4f+/-%.4f, p=%.4g\n', ...
	mean(meanAW_LWpos(vPN2)), std(meanAW_LWpos(vPN2))/sqrt(sum(vPN2)), ...
...
% --- Layout (normalized) ---
% Scatter bottom-left, top bar above, right barh to the right
LM = 0.16; BM = 0.14; RM = 0.04; TM = 0.06;
bwR = 0.24; bhT = 0.24; G = 0.06;

sW = 1 - LM - G - bwR - RM;   sH = 1 - BM - G - bhT - TM;
sX = LM;                        sY = BM;

% ===== E: Scatter (bottom-left) =====
...
% Shift labels to center across scatter + gap + bar panels
% xlabel: center of [sX, sX+sW+G+bwR] in axE normalized x
xlh.Units = 'normalized';
xlh.Position(1) = (sW/2 + G/2 + bwR/2) / sW;  % ~0.80
% ylabel: center of [sY, sY+sH+G+bhT] in axE normalized y
ylh.Units = 'normalized';
ylh.Position(2) = (sH/2 + G/2 + bhT/2) / sH;  % ~0.80

if ~isfinite(pCorr), pStr = 'p = NaN';
elseif pCorr < 0.001, pStr = 'p < 0.001';
elseif pCorr < 0.01, pStr = sprintf('p = %.3f', pCorr);
else, pStr = sprintf('p = %.2f', pCorr);
end
text(axE, 0.03, 0.97, pStr, 'Units', 'normalized', 'FontSize', fs, ...
	'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');

% ===== Top: bar (F1: split by 🔊💧 sign -> 💡💧 z-score) =====
...
% One-sided error bars: outward (upward, away from scatter)
for ib = 1:2
...
% Significance bracket (above both bars)
...
text(axT, 1.5, yBrk+0.005, iAsterisk(pPN1), 'FontSize', fs, ...
	'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
hold(axT, 'off');

axT.FontSize = fs;
axT.XTick = [1 2]; axT.XTickLabel = {'−', '+'};
axT.XAxisLocation = 'bottom';  % +/- on scatter side (bottom = closer to scatter)
axT.YAxisLocation = 'left';
box(axT, 'off');

% ===== Right: barh (F2: split by 💡💧 sign -> 🔊💧 z-score) =====
...
% One-sided error bars: outward (rightward, away from scatter)
for ib = 1:2
...
% Significance bracket (on outer/right side)
...
text(axR, xBrk+0.01, 1.5, iAsterisk(pPN2), 'FontSize', fs, ...
	'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Rotation', 270);
hold(axR, 'off');

axR.FontSize = fs;
axR.YTick = [1 2]; axR.YTickLabel = {'−', '+'};
axR.YAxisLocation = 'left';  % +/- on scatter side (left = closer to scatter)
