...
f = figure('Color', 'w', 'Name', 'English Fig1D Representative Cell Traces');
f.Units = 'centimeters';
f.Position(3:4) = [9, 8];
...
phaseNames = ["Naive🔊💧", "Learned🔊💧", "Transfer💡💧"];
trialColors = [1, 0, 0; 0, 0, 1; 0, 0, 0];
...
for iC = 1:3
	for iP = 1:3
		...
		for iT = 1:3
			h = plot(ax, xsPlot, traces(iT, :), 'Color', trialColors(iT, :), 'LineWidth', 2);
			if iC == 1 && iP == 1
				hLeg(iT) = h;
			end
		end
...
svgName = "English_Fig1D_RepresentativeCellTraces.svg";
svgPath = TransferLearning.ExportStandardFigure(f, 2, svgName);
fprintf('Wrote: %s\n', svgPath);

assignin('base', 'English_Fig1D_CellUIDs', selectedCellUIDs);
