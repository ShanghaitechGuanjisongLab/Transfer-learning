% 临时探索：L2/3、L5 × 两种异质性算法 的首会话命中率残差斜率相关性
% 算法A（当前正式版）：每细胞先跨会话平均，再在[-1,1]细胞中取SD
% 算法B（试验级）：每回合先在[-1,1]细胞中取SD，再在鼠内跨全部回合平均
% 对每个 tile，先用首会话命中率将 slope 做线性残差化，再与异质性做 Spearman 相关。
...
if ~ok1s
    error('scratch_C_corr_4tile_layer_algo_residfirsthit:No1s', 'Cannot find sample close to 1s in time axis.');
end
...
summaryRows = table('Size', [0 11], ...
    ...
    'VariableNames', {'Layer','Algorithm','NaiveN','TransferN','N','RhoResid','PResid','RhoRaw','PRaw','RhoFirstSD','PFirstSD'});

f = figure('Color', 'w', 'Name', 'Fig3C layer x algorithm residualized by first hit');
f.Units = 'centimeters';
f.Position(3:4) = [12, 8];
...
for iA = 1:numel(algorithms)
    ...
    for iL = 1:numel(layers)
        ...
        if nnz(use) >= 3 && std(firstHitAll(use)) > 0 && std(slopeAll(use)) > 0
        ...
        fprintf('Raw rho = %.6f, p = %.6g\n', rhoRaw, pRaw);
        fprintf('Residual rho = %.6f, p = %.6g\n', rhoResid, pResid);
        fprintf('FirstHit vs heterogeneity: rho = %.6f, p = %.6g\n', rhoFirstSD, pFirstSD);
        fprintf('Slope ~ firstHit linear fit: slope = %.6f * firstHit + %.6f\n', b(1), b(2));
        ...
        hN = scatter(ax, sdAll(maskN), slopeResid(maskN), 10, colorN, 'o', 'filled', 'LineWidth', 0.2);
        hT = scatter(ax, sdAll(maskT), slopeResid(maskT), 10, colorT, 's', 'filled', 'LineWidth', 0.2);
        if iA == 1 && iL == 1
            hLegend = [hN; hT];
        end

        if nnz(use) >= 2 && std(sdAll(use)) > 0 && std(slopeResid(use)) > 0
...
pngUNC = fullfile(outDirUNC, 'English_Fig3C_4Tile_LayerAlgorithm_ResidualizedByFirstHit.png');
pngLocal = fullfile(outDirLocal, 'English_Fig3C_4Tile_LayerAlgorithm_ResidualizedByFirstHit.png');
