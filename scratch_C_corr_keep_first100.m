outDirUNC = "\\Data-Server-2\个人数据\张天夫\202602"; %#ok<NASGU>
...
[slopeN, sdN, miceN] = iNaiveCohortDataKeepFirst100(DS_LAB, DS_LAI, idx1s);
[slopeT, sdT, miceT] = iCohortDataKeepFirst100(DS_Transfer, idx1s, "Transfer", "Final");

slopeAll = [slopeN; slopeT];
sdAll    = [sdN; sdT];
groupAll = [repmat("Naive", numel(miceN), 1); repmat("Transfer", numel(miceT), 1)]; %#ok<NASGU>
...
fprintf('Naive mice:\n');
disp(miceN');
fprintf('Transfer mice:\n');
disp(miceT');

function [slopeVec, sdVec, mice] = iNaiveCohortDataKeepFirst100(DS_LAB, DS_LAI, idx1s)
Sess = iGatherNaiveSessions(DS_LAB, DS_LAI);
Sess = iExcludeAudioWaterSessions(Sess, DS_LAB, DS_LAI);
Sess = iExcludeCeilingNaiveKeepFirst100(Sess);
[SessUsed, mice, slopeVec] = iPerMouseSlopeSessionsKeepFirst100(Sess);
...
function [slopeVec, sdVec, mice] = iCohortDataKeepFirst100(DS, idx1s, phaseStart, phaseEnd)
...
function [SessUsed, mice, slopeVec] = iPerMouseSlopeSessionsKeepFirst100(Sess)
if isempty(Sess)
	SessUsed = Sess;
	mice = string.empty(0,1);
	slopeVec = [];
...
for iM = 1:nMice
	...
	if ~isempty(first100) && first100 < height(R)
		R = R(1:first100, :);
	end
	if height(R) < 2, continue; end
	xi = (1:height(R))';
...
function AllSess = iExcludeCeilingNaiveKeepFirst100(AllSess)
...
for m = unique(AllSess.Mouse)'
	...
	if ~isempty(i100) && i100 < numel(rows)
		remove(rows(i100+1:end)) = true;
	end
end
AllSess(remove, :) = [];
perf = double(AllSess.Performance);
AllSess = AllSess(isfinite(perf) & perf >= -1e-12, :);
end
